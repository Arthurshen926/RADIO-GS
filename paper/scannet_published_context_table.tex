\begin{table*}[t]
\centering
\caption{ScanNet-v2 open-vocabulary point-cloud understanding under the
OpenGaFF/VALA protocol. External baseline rows are copied from the OpenGaFF v2
paper table and reported in percent; the OpenGaFF method row is intentionally
omitted. The \method{} row is our local VALA/OpenGaFF-8 evaluation on the same
eight scene ids.}
\label{tab:scannet_published_context}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lrrrrrr}
\toprule
Method & 19 mIoU & 19 mAcc & 15 mIoU & 15 mAcc & 10 mIoU & 10 mAcc \\
\midrule
LangSplat & 2.45 & 8.59 & 3.45 & 13.21 & 6.48 & 21.89 \\
LangSplatV2 & 14.75 & 25.47 & 17.09 & 35.68 & 22.83 & 41.52 \\
OpenGaussian & 27.73 & 42.01 & 29.67 & 46.15 & 39.93 & 57.34 \\
Dr. Splat & 29.31 & 47.68 & 33.25 & 54.33 & 44.19 & 65.19 \\
OccamLGS & 31.93 & 48.93 & 34.25 & 53.71 & 45.16 & 64.39 \\
VALA & 32.11 & 50.05 & 35.10 & 54.77 & 46.21 & 65.61 \\
\midrule
\method{} DINO-CV contextual kNN + spatial propagation & \textbf{38.06} & \textbf{61.29} & \textbf{38.71} & \textbf{63.15} & \textbf{47.11} & \textbf{72.00} \\
\bottomrule
\end{tabular}
}
\end{table*}
