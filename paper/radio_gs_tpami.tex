...
open-vocabulary query, \method{} reaches 58.89 mIoU and 85.98 Acc. On our
LERF-OVS direct 3D query-select-render implementation, compact primitive scoring
with support calibration reaches 50.14 mIoU and 70.44 Acc without
reading a multiview-registration cache or invoking an official RGB SAM decoder
at inference. On our ScanNet direct point-query evaluation over the VALA
eight-scene subset, the compact field reaches 38.06, 38.71, and 47.11 mIoU on
the 19-, 15-, and 10-class splits. Same-evaluator frame-wise
foundation-feature comparisons
...
are published source-paper values included as context. The local and published
readout protocols are not yet fully unified, so we do not treat these rows as a
...
\subsection{Canonical MPR and Global Surface-Region Readout}

\emph{Multiview Primitive Registration} (MPR) is query-free observation
lifting: posed teacher observations are fused into one compact canonical RADIO
field using visibility, coverage, and cross-view agreement.  It never consumes
text, object masks, benchmark labels, or a scene-specific classifier.  A
zero-mean low-capacity view residual may compensate only rendering context; all
text, image, and prompt interfaces still query the same canonical primitive
memory.

Dense SigLIP2 spatial tokens are not themselves reliable region summaries.
We therefore train one global surface-region readout on label-free ScanNet train
scenes and select it on disjoint official-validation scenes.  For primitive
$i$, a fixed physical-radius neighborhood $\mathcal{N}_r(i)$ is gathered from a
geometry/RGB-discontinuity surface graph.  The shared readout pools decoded
canonical RADIO features, relative 3-D geometry, and query-independent MPR
reliability,
\[
  u_i^r = R_\phi\!\left(\{D_\theta(c_j),\,g_{ij},\,\rho_j\}_{j\in
  \mathcal{N}_r(i)}\right), \qquad
  e_i = \mathrm{norm}\!\left(\frac{1}{|\mathcal R|}
  \sum_{r\in\mathcal R} A_{\mathrm{SigLIP2}}(u_i^r)\right).
\]
Here $A_{\mathrm{SigLIP2}}$ is the official frozen visual-summary head.  The
readout is trained once, is scene independent, and is frozen for every
benchmark.  Its targets are official C-RADIO crop summaries from generic
training surfaces; no benchmark vocabulary or evaluation image is opened.
Only the compact canonical RADIO field is persistent scene memory.  The
1536-dimensional $e_i$ values are disposable, query-independent evaluation
caches stored sparsely for observable surface primitives, rather than learned
per-scene semantic parameters.

\subsection{Query Modes and Support Calibration}

At evaluation time, every query is first compiled into evidence on the same
canonical primitives.  For a text query $q$, the default open-vocabulary score
is the query-set-invariant normalized cosine
\(s_i(q)=\cos(e_i,e_q)\).  Adding or removing an unrelated category therefore
cannot change an existing heatmap.  A scene-category softmax is retained only
as an explicitly named closed-taxonomy, paper-compatibility row.  Novel-view
outputs render the scalar primitive scores (or selected membership) with the
fixed geometry/visibility operator; they do not alpha-average a high-dimensional
semantic feature and then rescore it.  LERF-OVS reports localization accuracy
and mIoU over these rendered score maps.
...
\[
  ...
  s_i(q) = \cos(e_i, e_q),
\]
where $e_i$ is produced query-free by the global surface-region readout and the
frozen official SigLIP2 visual-summary head.  Registered 2-D and world-space
3-D prompts instead create positive/negative primitive seeds in the official
SAM3/DINOv3 capability spaces.  A fixed seeded random-walker may propagate
instance support on the shared surface graph while enforcing both seed signs.
Neither text scoring nor prompt propagation uses benchmark masks, labels,
per-scene thresholds, or test-set calibration.  RGB/depth discontinuities may
condition the query-independent observation topology, but do not change a
primitive's canonical query score.

\subsection{Typed Identity--Extent and Instance Readouts}

A single scalar similarity cannot simultaneously optimize sparse target
identity and complete object extent.  We therefore preserve two typed outputs
from the same persistent field.  For text query $q$, the canonical feature
defines an identity unary $u_i(q)=\cos(e_i,t_q)$.  Query-independent official
SAM proposals from legal source views are associated across views and lifted
with exact marginal responsibilities to instance memberships $r_{ik}$.  The
official SigLIP masked-crop representation scores proposal identity, giving
\begin{equation}
 p_i^{\mathrm{ext}}(q)=\sum_k p(k\mid q)r_{ik}.
\end{equation}
The extent branch is anchored to the field identity peak: it may complete the
support containing that peak but cannot move the localization result.  Identity
and extent maps are rendered and resized independently.  This readout adds no
second persistent semantic field.

The corresponding calibrated latent-variable form includes an explicit null
proposal,
\begin{equation}
 P(y_i=1\mid q,F)=\pi_0 p_i^{\mathrm{prim}}(q)
 +\sum_{k=1}^{K}\pi_k r_{ik},\qquad \sum_{k=0}^{K}\pi_k=1.
\end{equation}
The null branch preserves primitive evidence when no proposal is sufficiently
identified.  Cross-view edges are ternary (same, different, or unknown): an
unobserved co-membership is never treated as a negative.  We use the frozen
deterministic realization for the formal row.  Our analytic development
realization assigns each valid proposal a continuous cross-view support
$a_k$ and uses
$\ell_k=8(s_k^{\mathrm{id}}-b_q)+\log a_k-\log K_q$ with null logit zero.
The proposal-count prior prevents duplicated observations from overwhelming
the null.  On LERF3D full4 this changes mIoU/Acc@0.25/Acc@0.50 from
$0.39684/0.61538/0.40865$ to $0.43730/0.67788/0.48077$.  Because benchmark
feedback was already available during development, this is not presented as
an independent blind result; a learned calibrated replacement is reported
only after source-only cross-scene validation.

Soft membership is projected with the renderer's visibility measure,
$P(y_x=1\mid q)=\sum_iT_i(x)\alpha_i(x)P(y_i=1\mid q)$, retaining the residual
transmittance as background mass.  We do not alpha-normalize this probability,
which would magnify low-opacity tails.  A subsequent boundary module is allowed
to alter pixel support, but not proposal identity.
When the source-trained current-field SAM boundary module is composed with the
analytic latent posterior, LERF2D full-four sample-micro mIoU increases from
$0.37080$ to $0.37818$ while localization remains $0.87981$.  This positive
factorization diagnostic remains below the retained $0.39584$ readout and is
therefore not promoted as an unconditional replacement.

A label-opening full-four intervention ladder is used only for diagnosis.  Its
largest jump occurs when the finite proposal-set constraint is removed, while
oracle membership-threshold selection adds only about one sample-micro IoU
point after association.  We therefore treat proposal coverage and cross-view
association as method prerequisites and do not report oracle rows as results.

Mutually exclusive categories require a different posterior.  On ScanNet, SAM
affinity supplies only a bounded residual on rows whose top-two class margin is
below a fixed threshold,
\begin{equation}
 \tilde p_i=p_i+\alpha\mathbf 1[m_i<\tau]
 (L_{\mathrm{sam}}p-p)_i.
\end{equation}
High-margin class decisions are immutable, and SAM never creates category
identity.  For registered NVOS and SPIn prompts, protocol-authorized transient
RGB may be processed by frozen official SAM.  The shared primary compiler
freezes ten reference candidates, projects each through exact compositor
adjoints into every registered camera, performs every view call independently,
and symmetrically marginalizes all candidate/view evidence into one Gaussian
posterior.  Thus view order is not a hidden state; incomplete candidates or
views abstain instead of falling back.  A cheaper target-frame diagnostic
decodes box and signed-point hypotheses independently and combines them after
mask decoding using positive inclusion and negative exclusion.  We do not
treat a joint box--point prompt as additive evidence: that control fails the
frozen NVOS gate.  These three interfaces share the same Universal Field v1 but
respect the distinct statistical type of text, categorical, and
registered-prompt queries.
...
published source-paper values shown as context, whereas the \method{} rows are
linked to local source artifacts; source/provenance notes are retained for
auditability.
...
Table~\ref{tab:lerf_ovs_main} provides LERF-OVS 2D and 3D open-vocabulary
query results. Published baseline rows are included as context pending a fully
unified local rerun. The 2D rows evaluate rendered-view query
masks, while the 3D rows query Gaussian primitives first and render selected
support only for mask evaluation. For 2D query, the compact
RADIO feature field renders a dense RADIO-compatible feature map; the resulting
query heatmap is converted to a mask by the local fixed readout recorded in the
source artifact.
...
final 2D benchmark row. The final locally auditable 2D OVS row in
Table~\ref{tab:lerf_ovs_main} further uses the feature-only SAM3-adaptor
boundary head and reaches 58.89 mIoU.
...
records the deployed selector used for the local paper result. In all cases the
...
\label{tab:framewise_radio_vs_rendered}
\begin{tabular}{lcccc}
\toprule
...
Table~\ref{tab:framewise_radio_vs_rendered} supports the inference-time feature-memory
...
\texttt{scene0400\_00}, and \texttt{scene0590\_00}. We reuse the VALA scene and
class splits, but query at GT semantic point-cloud coordinates rather than
evaluating labels on pseudo-GT Gaussian centers. This is not presented as a
full ScanNet semantic segmentation leaderboard result or as a same-domain
comparison with the published rows; it is a split-aligned direct probe of the
learned feature field. The \method{} result uses
...
final argmax. It reaches 38.06/61.29,
38.71/63.15, and 47.11/72.00 mIoU/mAcc on the 19/15/10-class splits. The
baseline rows are published Gaussian-domain results shown as context and are
not claimed as local same-evaluator reproductions.
...
\method{} RADIO-compatible features and prompt embeddings; it does not call the official RGB
...
Fig.~\ref{fig:framewise_radio_field_qualitative} compares reconstructed scene features
...
\label{fig:framewise_radio_field_qualitative}
\end{figure*}
...
The main LERF and ScanNet quantitative tables separate locally auditable
\method{} rows from published source-paper context. Until all methods are rerun
under a single frozen protocol, the mixed-source tables are not used for strict
ranking claims.
...
benchmark. The main LERF and ScanNet tables separate local outputs from
published context; historical cross-paper provenance notes are kept for
auditability. The deployed compact direct-3D result uses GT-free color-edge
