...
\title{\method{}: Compact Foundation-Feature Fields for Open-Vocabulary 3D Gaussian Scene Understanding}
...
{\method{}: Compact Foundation-Feature Fields for Open-Vocabulary 3D Gaussian Scene Understanding}
...
\method{}, a compact foundation-feature field for 3D Gaussian scenes. Rather than
...
Contextual Gaussian Feature Field, reconstructs foundation-space features through a
Foundation-Space Reconstruction decoder, and exposes feature quality and visibility as
...
for cross-dataset semantic querying. Multiview Primitive Registration provides an
...
compact prompt-ensemble readout with label-free color-edge support calibration
reaches 0.5014 macro mIoU and 0.7044 Acc@0.25 without reading a multiview-registration cache
...
on the 19-, 15-, and 10-class splits. Same-evaluator frame-wise foundation-feature comparisons
and component ablations show that multiview reconstruction and support-calibrated direct 3D
readout drive the gains. Small fragmented objects remain the primary failure mode.
\end{abstract}
...
space before comparison to the frozen RADIO teacher. We also use Frozen-Head
...
object-query setting. External results are used as source-anchored context when
...
\begin{itemize}
  \item We formulate compact foundation-feature reconstruction for queryable 3D
  Gaussian scene understanding across rendered-view localization, direct
  Gaussian object selection, and direct point queries.
  \item We introduce a unified multi-head Contextual Gaussian Feature Field that stores
  compact per-Gaussian codes, augments them with spatial context, and predicts
  feature quality and visibility without direct high-dimensional foundation-feature
  storage.
  \item We use Foundation-Space Reconstruction, View-Conditioned Feature
  Calibration, Frozen-Head Geometry Consistency, and
  multiview-registration-to-field transfer to recover teacher-compatible
  features while keeping teachers and downstream heads fixed; multiview
  registration acts as a registered supervision bridge rather than a required
  inference cache for the compact direct readout.
...
compact primitive scores with a support-calibrated primitive readout for LERF direct 3D object
selection, and direct point features for ScanNet queries. Frozen
RADIO/SigLIP2/DINOv3/SAM3 heads, multiview-registration-to-field support distillation, and
geometry/visibility losses are training constraints; the main compact direct
readout does not require a multiview-registration feature cache or an official RGB SAM decoder.}
\label{fig:framework}
\end{figure*}
...
\subsection{Multi-Head Foundation Consistency}
...
ensemble and a Support-Calibrated Primitive Readout: selected Gaussians are rendered
to a coarse mask, label-free color-edge support calibration refines local boundaries, and a component
...
floor. This is
a label-free readout rule. It uses the evaluation RGB image only through a
...
We also retain \emph{Multiview Primitive Registration} as a supervision bridge.
It renders teacher-space features from posed views, applies view-conditioned calibration and
...
label-free multiview-registration-to-field consistency variant. Let $\bar{e}^{\mathrm{mpr}}_i$ be
the cached normalized summary feature registered to Gaussian $i$, and
let $n_i$ be the number of posed views that registered valid evidence to
Gaussian $i$. The transfer objective uses a label-free confidence weight
...
\]
where $\mathcal{L}_{text}$ matches the registered multiview scene-query distribution
...
readout does not read the registration cache at inference; the frozen SAM3 box-prompt
...
the class splits described below. External baseline results are official-source
values from the cited papers or supplements and are captioned as cross-paper
context rather than reproduced same-evaluator results; the exact source results
...
The LERF, LangSplat, and LEGaussians results are traced to official paper or
supplement tables and converted to decimal LocAcc when originally reported as
percentages. Because these results are not rerun under the \project{} evaluator, we
...
feature quality. Raising the label-free peak-relative threshold from 0.50 to 0.60
...
ensemble raises this to 0.457/0.685, while label-free color-edge and
score-component support calibration reaches 0.501/0.704. The gain is largest on Waldo Kitchen, where
Acc@0.25 rises from 0.409 for the earlier compact readout to 0.545 by recovering
small or fragmented object support. This makes the deployed compact direct readout stronger
than the registered multiview readout in Acc@0.25 without reading a multiview-registration feature cache
...
boundary gap, but the core direct-field claim is carried by the compact readout.
...
results used by the recent OpenGaFF paper, with the OpenGaFF method result omitted by
design. These results are deliberately separated from the local result table: they
...
selection requires Support-Calibrated Primitive Readout; query-level audits identify
...
SigLIP2 head, scene-category scoring, and label-free threshold-0.60 plus
...
foundation-space reconstruction codec. Readout calibrations such as peak-component filtering, feature-only
...
supplementary evidence records additional variants such as frozen official SAM3 box
...
polygons. The paper therefore keeps the calibrated LERF readout as the main
...
external results are copied from the OpenGaFF paper but the OpenGaFF method result is
...
Support-Calibrated Primitive Readout; they do not read a multiview-registration feature cache and do not call the
...
prior result uses reproduced local baselines: LangSplat for 2D OVS and Dr. Splat
for 3D OVS. \method{} uses one compact foundation-feature Gaussian map for both
readouts; the 3D panels do not use a multiview-registration cache or official RGB SAM3 readout.}
...
ablation on small or fragmented LERF queries. The base compact readout
...
compact direct readout.

\begin{figure*}[t]
\centering
\includegraphics[width=\linewidth]{figures/lerf_direct3d_support_policy_ablation_qualitative.png}
\caption{Qualitative ablation of the compact direct-3D support-calibrated readout. The
readout recovers small or fragmented object support without a multiview-registration feature
...
readout is needed to convert primitive scores into stable object masks. Bare
...
The LERF, LangSplat, and LEGaussians comparison results are traced to official
paper or supplementary tables. They still come from cross-paper protocols, so
we treat them as source-anchored context. A strict ranking claim would
...
regularization, and support-calibrated readouts can suppress view-specific noise and
...
LERF direct 3D object-selection analysis substantially improves primitive querying
and exceeds the OpenGaussian official macro reference under the reported
OpenGaussian-style context, but the external results are not locally rerun under one
evaluator; universal primitive-level ranking claims would require those reruns.
The deployed compact direct-3D readout uses label-free color-edge and
score-component support calibration. It avoids multiview-registration cache
inference and official RGB SAM decoding, but it is not the strict no-RGB
ablation; that stricter readout remains lower at 0.457 macro mIoU. The frozen
official SAM3 box result is therefore kept as a post-selection boundary readout
rather than the core direct-field claim. The storage table
