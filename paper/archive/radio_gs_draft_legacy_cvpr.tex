...
64.98 mIoU and 82.68 Acc across four scenes. On the VALA/OpenGaFF eight-scene
...
reaches 36.55, 42.78, and 57.85 mIoU on the 19-, 15-, and 10-class splits. An
...
reaches 54.36 mIoU and 80.84 Acc, without reading a
...
The paper-facing \project{} numbers come from the current fixed evaluation
artifacts and reproduced benchmark tables. The RADIO adaptor ablations were
completed under the same LERF sweep protocol. Baseline rows in the main
quantitative tables follow the same reproduced evaluation protocols as
\method{}; historical source and provenance notes are recorded separately for
auditability.
...
Scene & Acc & mIoU & Temp. \\
\midrule
Figurines & 92.86 & 64.29 & 50 \\
Ramen & 62.83 & 53.78 & 40 \\
Teatime & 93.22 & 76.09 & 25 \\
Waldo Kitchen & 81.82 & 65.76 & 25 \\
\midrule
Macro & 82.68 & 64.98 & -- \\
\bottomrule
\end{tabular}
...
Table~\ref{tab:lerf} is the current same-protocol rendered-view 2D query result.
The compact RADIO feature field renders a dense RADIO-compatible feature map,
and the fixed query-mask protocol is shared with the reproduced baseline rows.

\input{lerf_ovs_main_table}

Table~\ref{tab:lerf_ovs_main} provides the same-protocol LERF-OVS 2D and 3D
open-vocabulary query comparison. The 2D rows evaluate rendered-view query masks,
while the 3D rows query Gaussian primitives first and render selected support
only for mask evaluation.
...
diagnostic readout after compact 3D selection remains in the appendix as a
boundary upper-bound analysis. The main direct-3D comparison is the reproduced
LERF-OVS 3D OVS table, where \method{} achieves 54.36 mIoU and 80.84 Acc.

\input{lerf_direct_3d_context_table}

Table~\ref{tab:lerf-direct-3d-context} gives historical baseline provenance
used during project development. The main ranking claims use the reproduced
2D/3D LERF-OVS table.
...
baseline rows follow the same reproduced OpenGaFF/VALA protocol and the
OpenGaFF method row is omitted by design. Increasing the calibration scale to 0.75
...
only as auxiliary audit material.
...
submission path: strong LERF-OVS same-protocol results, ScanNet direct-query
evidence, explicit storage and runtime profiles, and failure analysis.
