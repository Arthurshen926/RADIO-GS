...
0.4293 mIoU on the 19-, 15-, and 10-class splits, respectively. An
OpenGaussian-style direct 3D object-selection stress test exposes a remaining
primitive-level gap, where pre-refiner Gaussian-center features are queryable
but not yet competitive with clustered language-Gaussian methods. These results
support the central claim that 3D Gaussian scenes can serve as reusable
foundation-feature memories, while clarifying that strong primitive-level
selection still requires direct 3D supervision or instance aggregation.
\end{abstract}
...
Gaussians that are then rendered only for mask evaluation~\cite{wu2024opengaussian}.
\method{} differs
...
For direct 3D querying, we bypass the VFA screen-space refiner and decode
pre-refiner compact features at Gaussian centers:
\[
...
experiments, but without rendered-view alignment.
...
as an ablation/selector result rather than replacing the main row.
...
Table~\ref{tab:lerf-direct-3d-selection} evaluates the missing primitive-level
bridge to 2024--2025 open-vocabulary 3DGS work. We follow the OpenGaussian-style
query-select-render protocol: text similarity is computed on Gaussian-center
features, selected primitives are rendered to official LERF-OVS annotated views,
and metrics are mIoU and Acc@0.25. This protocol is not the same as the
rendered-view heatmap evaluation in Table~\ref{tab:lerf}; it is a direct 3D
object-selection stress test.

The result is a useful negative finding. Pre-refiner Gaussian-center features
are directly queryable, but they are not yet competitive with instance- or
cluster-aware language Gaussian methods. The fixed top-10\% primitive selector
reaches 0.080 macro mIoU and 0.093 macro Acc@0.25, far below OpenGaussian's
reported 0.384/0.514. Even a diagnostic best-by-scene ratio sweep only reaches
0.091 macro mIoU. Additional no-GT diagnostics with KNN point readout,
semantic/geometry scoring heads, scene-softmax calibration, adaptor-promoted
checkpoints, and voxel score aggregation do not improve the fixed-protocol macro
mIoU. We therefore keep LERF direct selection as an aligned protocol and
limitation, not as a main SOTA claim; improving it likely requires direct 3D
language supervision or instance-aware primitive aggregation.
...
claim: the compact field supports a direct Gaussian-center readout, but the
current readout lacks the primitive grouping needed for strong object selection.
...
not a replacement for a full standardized segmentation benchmark. The new LERF
direct 3D object-selection result is substantially weaker than OpenGaussian-style
clustered primitive selection, so the paper should not claim primitive-level
SOTA without additional direct 3D supervision or instance aggregation. Finally,
