...
rendered-feature-to-primitive registration readout raises fixed-protocol macro
mIoU from 0.0804 to 0.3421 and macro Acc@0.25 from 0.0932 to 0.5547. These results
support the central claim that 3D Gaussian scenes can serve as reusable
foundation-feature memories across rendered-view and direct-primitive readouts,
while clarifying that Waldo Kitchen remains a difficult object-fragmentation case.
\end{abstract}
...
For direct 3D querying, the base readout decodes pre-refiner compact features
at Gaussian centers:
\[
...
experiments. For LERF direct object selection, we also evaluate a registration
readout inspired by direct language-registration methods: render teacher-space
features from posed views, apply VFA and the frozen SigLIP2 head, register those
text-aligned features back to visible Gaussian centers with depth/alpha checks,
and then run the same primitive-level text query on the registered Gaussian
embeddings. The LERF masks are used only for final query-select-render
evaluation, not for registration or scoring.
...
The original Gaussian-center readout reached only 0.080 macro mIoU and 0.093
macro Acc@0.25. The registration readout closes most of this gap without using
LERF masks for scoring: with a fixed top-2\% primitive selector it reaches
0.342 macro mIoU and 0.555 macro Acc@0.25, and its best-by-scene diagnostic
reaches 0.362/0.610. This remains below OpenGaussian's official 0.384 mIoU
because Waldo Kitchen is still weak, but it is competitive on Ramen and improves
primitive-level usability by more than four times over the original direct
readout.
...
but remains weaker than OpenGaussian on Waldo Kitchen, so the paper should avoid
