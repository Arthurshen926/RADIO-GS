...
floor and 2\% top-ratio cap reaches 0.4133 macro mIoU and 0.6741 macro
Acc@0.25. For context, the raw Gaussian-center
...
fixed top-2\% selector used by the VPR rows. The registration readout closes
...
0.385/0.643, while the GT-free mean+2.5std score-distribution selector with a
0.5\% floor and 2\% top-ratio cap reaches 0.413 macro mIoU and 0.674 macro
Acc@0.25. Its best-by-scene diagnostic reaches 0.417/0.674. This is above the OpenGaussian official macro reference while
using a different SigLIP2 text head and no locally rerun baseline reproduction;
Waldo Kitchen also shows that explicit instance grouping can still be valuable
on fragmented objects. We therefore use the result as evidence for registered
...
registration step uses posed rendered views and visibility checks, but no LERF
...
whereas 0.012/0.009 is the same top-2\% selector used by the VPR rows.
...
support only, DINO mask propagation uses a fixed source-background contrast
weight of 0.5, and no external SAM3 mask decoder is called.}
...
DINOv3 mask propagation & 0.7163 & 0.3921 & 0.7376 & 0.3684 \\
\bottomrule
\end{tabular}
}
\end{table}
...
propagation substantially stronger and gives rendered features higher
propagation LocAcc than the teacher, but the rendered mask mIoU remains slightly
lower. The higher rendered dense-matching similarity score suggests that the
feature field can become over-smooth and select visually similar nearby regions.
This motivates the cross-view DINO and text-heatmap protected training variants
evaluated next.
...
rendered mask from top-2\% VPR-selected Gaussians, and an error map. Green is
true positive, red is false positive, and blue is false negative. The Waldo
Kitchen row illustrates the remaining fragmentation and clutter failure mode.}
