...
ScanNet direct point-query protocol, \method{} reaches 0.3538, 0.3573, and
...
\subsection{RADIO Adaptor Consistency}
...
The conservative baseline numbers come from the 2026-05-02 submission-freeze
report:
...
protocol. External baseline rows are not yet treated as final claims because
...
component contributes under the same evaluator: the hybrid field tests compact
feature storage, HCD tests the nonlinear bottleneck codec, the screen-space
refiner tests peak and boundary correction, and the FDH warm-start tests the
geometry-aware training stage.

The largest dependency is the HCD codec. Replacing HCD with a direct $1\times1$
projection drops macro LocAcc from 0.858 to 0.531 and macro mIoU from 0.485 to
0.260, with every scene degraded. FDH warm-start is the next strongest
component, improving macro LocAcc by 0.056 and macro mIoU by 0.061 over the
no-FDH run. The refiner gives a smaller but consistent localization gain
($+0.018$ macro LocAcc) with almost unchanged mIoU. The hybrid representation
