...
aggregation raises fixed-protocol macro mIoU from 0.0804 to 0.3850 and macro
Acc@0.25 from 0.0932 to 0.6428. These results
...
LERF masks for scoring. With GT-free voxel-max context aggregation and a fixed
top-2\% primitive selector it reaches 0.385 macro mIoU and 0.643 macro
Acc@0.25, and its best-by-scene diagnostic reaches 0.397/0.665. This is slightly
above the OpenGaussian official macro reference while using a different SigLIP2
text head and no locally rerun baseline reproduction; Waldo Kitchen and Teatime
also show that explicit instance grouping can still be valuable on fragmented
objects. We therefore use the result as evidence for registered primitive-level
usability rather than as a universal primitive-level SOTA claim.

\input{lerf_vpr_ablation_table}

Table~\ref{tab:vpr_ablation} audits the implementation choices. The raw
Gaussian-center score is nearly unusable under the same fixed top-2\% selector.
Registering VFA-refined rendered features back to visible primitives is the main
gain; disabling VFA collapses the result, and removing the depth check registers
almost every Gaussian but does not improve mIoU. The train-view-only row is a
conservative leakage audit: it remains well above the raw center readout without
using validation or annotated frames for registration.
