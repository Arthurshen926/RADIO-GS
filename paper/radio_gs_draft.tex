...
Acc@0.25. A strict no-RGB one-map ablation reaches 0.457 macro
mIoU and 0.685 macro Acc@0.25, showing that the strongest deployed row owes its
boundary/support recovery to the guard rather than to VPR or official SAM3. The
...
The main qualitative comparison in Fig.~\ref{fig:qualitative} uses one fixed
annotated view from each LERF scene and follows the same direct-3D query
interface as Table~\ref{tab:lerf-direct-3d-selection}: the query is evaluated on Gaussian
primitives and rendering is used only to visualize and score the selected 3D
objects. We compare against our local Dr. Splat reproduction because it is a
representative registration-style baseline for direct 3D object selection. The
\method{} panels use the deployed compact field only; they do not read a VPR
feature cache and do not call the official RGB SAM3 readout at evaluation time.

\begin{figure*}[t]
\centering
\includegraphics[width=\linewidth]{figures/lerf_main_qualitative_comparison.png}
\caption{Main LERF qualitative comparison under the direct-3D object-selection
protocol. Each row fixes a scene, annotated frame, and text query, then shows
the RGB input, ground-truth mask, a reproduced Dr. Splat mask, and our compact
direct-field mask. IoU is shown per panel for auditability. \method{} queries
the compact foundation-feature Gaussian map directly; VPR is used only as a
training teacher and no official RGB SAM3 boundary readout is used for these
predictions.}
...
main-paper figures, since their protocols differ from direct 3D object
selection.
