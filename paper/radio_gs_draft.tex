...
compact direct field. On LERF-OVS, the current frozen \project{} package with a
...
compressed back into the queryable Gaussian-center field. Its bare score
thresholding still trails streamed registered VPR, whereas the strongest strict
paper-facing direct-field row combines compact primitive scoring with the frozen
SAM3 box-prompt boundary readout described below.
...
baseline reproduction. In the published-context table copied from recent
...
rendered \method{} features win 5 of 6 primary selected downstream metrics.
This summary is intentionally phrased as selected downstream feature usability,
because DINOv3 mask-propagation mIoU and dense-match hit rate remain stronger
for the frame-wise teacher under the same readout.
...
Additional alpha/depth boundary cases and unconstrained adaptor-matching
qualitatives are retained as appendix artifacts rather than main-paper figures.
...
consistency improves the direct student field substantially, but bare
score-thresholding does not yet fully replace the streamed registered VPR
readout on all scenes; the strongest reported strict row uses the compact direct
field together with a frozen SAM3 boundary readout. Finally,
