...
improves them to 0.3677, 0.3748, and 0.4562. An
...
The contextual row uses kNN point readout with $k=8$, 32 candidate Gaussians,
and label-free scene-mean logit calibration with $\alpha=0.5$.}
\label{tab:scannet}
\begin{tabular}{llcc}
\toprule
...
0.3677, 0.3748, and 0.4562 while keeping mAcc comparable. Increasing the
calibration scale to 0.75 further raises the 10-class mIoU to 0.4585 but lowers
...
Under this protocol, the contextual \method{} row slightly exceeds the OpenGaFF
published 19-class mIoU and is stronger than prior pre-OpenGaFF rows on mAcc,
but remains below OpenGaFF on the 15-/10-class mIoU and 10-class mAcc. We
...
to the VALA/OpenGaFF ScanNet-8 subset. DINO cross-view affinity improves the macro
