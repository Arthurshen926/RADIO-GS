...
\title{Supplementary Material for ``\method{}: Compact Teacher Feature Fields for Open-Vocabulary 3D Gaussian Scene Understanding''}
...
point querying, and frozen-head teacher comparisons. The tables below provide
...
peak-connected-component readout are GT-free and use the heatmap argmax only to
...
querying. VPR is treated as a registered multiview supervision and analysis
bridge, while the main compact direct row in the paper uses Gaussian-center
primitive scores with a fixed support-aware policy and does not read a VPR
feature cache at inference.
...
the central design choice of reconstructing compact teacher-compatible features
rather than storing a raw high-dimensional teacher vector per primitive.
...
the SAM3-box boundary readout. This readout is reported as a boundary-assisted
control rather than the core compact direct row.
...
\input{train_feature_field_audit_table}

\clearpage
\section{Additional Qualitative Results}

Figure~\ref{fig:supp_sam_dino} shows frozen-head adaptor examples. These
examples are useful for interpreting the teacher-vs-student benchmark, but the
...
Figure~\ref{fig:supp_vpr_direct3d} shows the registered VPR direct-3D control.
It is included as an analysis readout because VPR transfers registered multiview
support to primitives, whereas the main compact direct row uses the deployed
compact Gaussian-center field.

\begin{figure}[t]
\centering
\includegraphics[width=0.86\linewidth]{figures/lerf_vpr_direct_3d_qualitative.png}
\caption{Registered VPR direct-3D control examples. VPR is used as an analysis
and training bridge, not as the main compact-field inference cache.}
...
row illustrates a boundary readout after compact primitive selection and is
