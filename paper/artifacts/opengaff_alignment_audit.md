...
| LERF direct 3D object selection | Query 3D primitives, render selected primitives, evaluate mIoU / Acc@0.25 on LERF-OVS masks | VPR fixed row `0.4801` / `0.6760`; direct-field + official SAM3-box fixed row `0.5705` / `0.6835` | Compare against the published public baseline rows only; avoid any OpenGaFF head-to-head claim. |
