...
- nearest-view / cached-teacher baseline under the same LERF evaluator;
- full per-Gaussian 1280-D explicit teacher-feature baseline under the same
  LERF evaluator.

## Search Result

The nearest-view cached-teacher baseline is now measured in
...
The full per-Gaussian 1280-D explicit teacher-feature baseline is now measured
...
registers cached RADIO teacher features to visible Gaussian centers, stores the
...
|---|---|
| `output/radio_gs/reports/controlled_evidence_table.md` | Includes both measured controlled rows; use its source column to distinguish cache-only, explicit 1280-D scene memory, and compact CTF-GS. |
...
| `output/radio_gs/reports/lerf_component_ablation.md` | Includes compact architectural ablations such as w/o hybrid/HCD, but not full raw 1280-D per-Gaussian teacher features. |
...
| frame-wise RADIO teacher row | This is a 2D teacher/reference row, not a nearest-view 3D cache or explicit per-Gaussian 1280-D scene baseline. |
...
the row below the frame-wise teacher and nearest-view cache controls, and above
the compact CTF-GS row, because it has 3D memory but is not compact and only
...
The controlled evidence table now covers teacher, nearest-view cache, full
per-Gaussian 1280-D explicit scene memory, full compact CTF-GS, core ablations,
