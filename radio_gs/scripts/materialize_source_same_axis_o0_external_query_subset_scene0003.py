#!/usr/bin/env python3
"""Materialize the frozen label-blind same-axis O0 subset for scene0003."""
...
AUTHORITY_SCHEMA = "radio_gs.source_same_axis_o0_scene0003_subset_authority.v1"
OUTPUT_SCHEMA = "radio_gs.source_same_axis_o0_query_subset.v1"
SCENE_ID = "scene0003_00"
SPLIT = "source_external_validation_multisource_selector_only"
...
def validate_execution_authority(value: object) -> dict[str, Any]:
    ...
    if not isinstance(value, Mapping) or set(value) != required:
        raise ValueError("scene0003 same-axis O0 subset authority fields differ")
    authority = dict(value)
    if (
        authority.get("schema") != AUTHORITY_SCHEMA
        or authority.get("schema_version") != 1
        or authority.get("status")
        != "sealed_before_scene0003_label_blind_query_subset"
        or authority.get("scene_id") != SCENE_ID
        or authority.get("split") != SPLIT
        or authority.get("frozen_method") != _method()
        or authority.get("source_access") != _access()
        or authority.get("benchmark_execution_authorized") is not False
    ):
        raise ValueError("scene0003 same-axis O0 subset authority header differs")
    for name in (
        "implementation",
        "frozen_selection_implementation",
        "accepted_v2",
    ...
    if not isinstance(banks, list) or len(banks) != 5:
        raise ValueError("scene0003 same-axis O0 text banks differ")
    clean_banks = []
    for row in banks:
        if not isinstance(row, Mapping) or set(row) != {
            ...
            raise ValueError("scene0003 same-axis O0 text-bank record differs")
        clean_banks.append(
            {
    ...
    authority["output"] = str(Path(authority["output"]).expanduser().resolve())
    return authority
...
def materialize(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("scene0003 AcceptedV2 axes differ")
    positive_parts: list[torch.Tensor] = []
    positive_records: list[dict[str, Any]] = []
    offset = 0
    for bank_record in authority["text_banks"]:
        bank = frozen_api._load_bank(bank_record, label=bank_record["component_id"])
    ...
    payload = {
        ...
        "canonical_negative_query_count": 4,
    ...
    return {
        "status": "scene0003_same_axis_O0_query_subset_complete",
        "output": file_record(output),
        "positive_queries": len(positive_ids),
        "canonical_negative_queries": 4,
