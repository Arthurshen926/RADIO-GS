...
SCENE="$1"
REPO=/root/RADIO-GS
ROOT="$REPO/local_ssd_results/lerf_full4_odds_residual_v4_corrected_20260810"
PREREG="$REPO/paper/artifacts/lerf_full4_post_readout_odds_residual_transport_v4_preregistration_20260810.json"
CORRECTION="$REPO/paper/artifacts/lerf_full4_post_readout_odds_residual_transport_v4_implementation_correction_20260810.json"
[[ "$(sha256sum "$PREREG" | awk '{print $1}')" == 859b0c8fb91008ee534649811d82733817f765832de95a5349deb428d549e62b ]]
[[ "$(sha256sum "$CORRECTION" | awk '{print $1}')" == 72c62a3063c619cac4b5bcf90aaa72e46c341b9179ff4bf1b5c9df2011c8b812 ]]
[[ "$(sha256sum "$REPO/radio_gs/querying/lerf_source_text_likelihood.py" | awk '{print $1}')" == 2a4e393f1cbce5ca62bd772b6e783571fd1cdaa5fc947698fc870c50878bec30 ]]
[[ "$(sha256sum "$REPO/radio_gs/scripts/eval_lerf_direct_3d_selection.py" | awk '{print $1}')" == e9e5988c2a37a90de7cee03f013097d5d98d53e15147811861403aed21342803 ]]
...
mkdir -p "$ROOT"
OUTPUT="$ROOT/pred_${SCENE}_v4"
RECEIPT="$ROOT/pred_${SCENE}_v4.receipt.json"
LOG="$ROOT/pred_${SCENE}_v4.log"
...
/root/miniconda3/envs/cybersim_agent/bin/python \
  ...
  --ours_source_text_likelihood_odds_residual_cache "$LIKELIHOOD" \
  --prediction_only \
