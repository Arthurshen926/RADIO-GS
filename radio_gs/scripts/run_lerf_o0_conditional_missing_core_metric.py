#!/usr/bin/env python3
"""Preregister and run one frozen LERF metric for a passed FIX6b cache."""
...
from radio_gs.interfaces import lerf_o0_conditional_missing_core_completion as formal
from radio_gs.scripts import (
    materialize_lerf_o0_conditional_missing_core_completion as materializer,
)
...
SCHEMA = "radio_gs.lerf_o0_conditional_missing_core_frozen_metric.v1"
STATUS = "authorized_single_FIX6b_source_frozen_conditional_metric"
...
FROZEN_INPUTS = {
    ...
    "renderer_geometry_checkpoint": {
        "path": (
            "/mnt/pool/sqy/results/RADIO-GS/output/radio_gs/"
            "lerf_figurines_v14_fdh_ws240_240ep/checkpoints/best.pth"
        ),
...
def _build_access() -> dict[str, bool]:
    return {
        "conditional_authority_cache_report_and_O0_lineage_validated": True,
        "source_train_and_strict_heldout_PASS_bound": True,
...
def _load_passed_conditional(
    *,
    ...
    authority_raw, authority_digest, authority_path = load_json_object(
        ...
        label="FIX6b cache execution authority",
    )
    authority = materializer.validate_authority(authority_raw)
    source_threshold = materializer._validate_source_gate(
    ...
    cache_raw, cache_digest, cache_path = load_torch_mapping(
        ...
        map_location="cpu",
        label="FIX6b external query score cache",
    )
    ...
    if (
        ...
        or report.get("status") != "conditional_missing_core_no_GT_gate_passed"
        or report.get("source_heldout_status")
        ...
        or cache["metadata"]["scene_id"] != "figurines"
        or cache["metadata"]["input_authority"] != authority["input_authority"]
        or cache["metadata"]["frozen_threshold_inclusive"]
        != materializer.FROZEN_THRESHOLD
        or source_threshold != materializer.FROZEN_THRESHOLD
        or cache["metadata"]["candidate_units"] != report.get("candidate_units")
        or cache["metadata"]["selected_units"] != report.get("selected_units")
        or cache["metadata"]["selected_unique_cells"]
        != report.get("selected_unique_cells")
        or cache["metadata"]["strictly_changed_cells"]
        != report.get("strictly_changed_cells")
        or int(report.get("threshold_membership_flips", -1)) <= 0
    ):
        raise ValueError("FIX6b passed no-GT binding differs")
    o0_record = authority["input_authority"]["exact_o0_cache"]
    o0, _, _ = load_torch_mapping(
        o0_record["path"],
        expected_sha256=o0_record["sha256"],
        map_location="cpu",
        label="FIX6b exact O0 parent",
    )
    metadata = o0.get("metadata", {})
    if (
        o0.get("schema")
        != "radio_gs.lerf_o0_anchored_graph_residual_external_scores.v1"
        or metadata.get("canonical_capability")
        != "exact_frozen_O0_canonical_negative_VALA_peak_scale"
        or metadata.get("renderer_geometry_checkpoint")
        != FROZEN_INPUTS["renderer_geometry_checkpoint"]
        or list(metadata.get("query_names", ())) != cache["metadata"]["query_names"]
        or not torch.equal(torch.as_tensor(o0.get("valid")), cache["valid"])
        or not torch.equal(torch.as_tensor(o0.get("xyz")).float(), cache["xyz"])
    ):
        raise ValueError("FIX6b O0/frozen renderer/query lineage differs")
    return authority, cache, report


def build_authority(args: argparse.Namespace) -> dict[str, Any]:
    output = _new(args.output_authority, label="FIX6b metric authority")
    ...
    cache_record = _record(
        ...
        label="FIX6b external query cache",
    )
    ...
    _load_passed_conditional(
        authority_record=authority_record,
        cache_record=cache_record,
        report_record=report_record,
    )
    frozen = {
        name: _mapping_record(record, label=f"FIX6b {name}")
    ...
    value = {
        ...
        **frozen,
        "label_root": _unopened(args.label_root, label="FIX6b label root"),
        "output_dir": _unopened(args.output_dir, label="FIX6b metric output"),
        "protocol": PROTOCOL,
    ...
    return {
        "status": "single_FIX6b_metric_preregistered_before_GT_access",
...
def validate_authority(path: str | Path, *, expected_sha256: str) -> dict[str, Any]:
    raw, digest, source = load_json_object(
        ...
        label="FIX6b frozen metric authority",
    )
    ...
    if (
        ...
        or value.get("conditional_materializer")
        != file_record(materializer.IMPLEMENTATION)
        ...
        raise ValueError("FIX6b metric authority header differs")
    for name, expected in FROZEN_INPUTS.items():
        if _mapping_record(value[name], label=f"FIX6b metric {name}") != expected:
            raise ValueError("FIX6b frozen metric input differs")
    authority_record = _mapping_record(
        value["cache_execution_authority"], label="FIX6b metric cache authority"
    )
    cache_record = _mapping_record(
        value["external_query_score_cache"], label="FIX6b metric cache"
    )
    report_record = _mapping_record(
        value["no_GT_report"], label="FIX6b metric report"
    )
    _load_passed_conditional(
        authority_record=authority_record,
        cache_record=cache_record,
