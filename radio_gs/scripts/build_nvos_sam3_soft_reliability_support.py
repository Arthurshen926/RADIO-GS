#!/usr/bin/env python3
"""Build the registered posthoc entropy-reliability NVOS support."""
...
ARTIFACT_TYPE = "nvos_sam3_entropy_reliability_query_conditioned_support"
SCHEMA_VERSION = 1
REGISTRATION_SHA256 = (
    "fcff94822697af71a8ac85ece28935f34b181c626070414ca25566b5614dc97f"
)
METHOD_CONTRACT = {
    "claim": "posthoc_registered_followup_not_independent_validation",
    "track": "source_reference_RGB_official_SAM3_soft_entropy_reliability",
    ...
    "positive_observation": "q_times_c(q)_raw_positive_overwrite_1_raw_negative_overwrite_0",
    "negative_observation": "raw_official_negative_scribble_separate_exact_channel",
    "initial_evidence": "signed_exact_W_transpose_soft_positive_minus_raw_negative_divided_by_W_transpose_one",
    "classifier": "balanced_logistic_C_0.01_signed_nonzero_rows_weighted_by_soft_positive_plus_negative_W_transpose_mass",
    ...
    "Li_threshold": "forbidden",
    "target_dependent_tuning": False,
}
PIXEL_TENSOR_KEYS = {
    "aggregate_probability",
    "binary_entropy_reliability",
    "soft_positive_observation",
    "raw_positive",
    "raw_negative",
}
TENSOR_KEYS = {
...
def _validate_pixel_tensors(
    pixel_tensors: object,
    *,
    authority: PromptResponsibilityAuthority,
) -> dict[str, str]:
    if not isinstance(pixel_tensors, dict) or set(pixel_tensors) != PIXEL_TENSOR_KEYS:
        raise ValueError("soft reliability pixel tensor schema differs")
    shape = (int(authority.height), int(authority.width))
    for name in (
        "aggregate_probability",
        "binary_entropy_reliability",
        "soft_positive_observation",
    ):
        value = pixel_tensors[name]
        if (
            ...
            or bool(((value < 0) | (value > 1)).any())
        ):
            raise ValueError(f"soft reliability pixel tensor {name} is malformed")
    for name in ("raw_positive", "raw_negative"):
        value = pixel_tensors[name]
        if (
            not torch.is_tensor(value)
            or value.device.type != "cpu"
            ...
            raise ValueError(f"soft reliability pixel mask {name} is malformed")
    reliability, observation = entropy_reliability_soft_observation(
        pixel_tensors["aggregate_probability"].numpy(),
        pixel_tensors["raw_positive"].numpy(),
        pixel_tensors["raw_negative"].numpy(),
    )
    if not torch.equal(
        pixel_tensors["binary_entropy_reliability"],
        torch.from_numpy(reliability),
    ):
        raise ValueError("stored entropy reliability differs from registered formula")
    if not torch.equal(
        pixel_tensors["soft_positive_observation"],
        torch.from_numpy(observation),
    ):
        raise ValueError("stored soft observation differs from registered formula")
    return {
        name: tensor_sha256(value) for name, value in sorted(pixel_tensors.items())
    }


def validate_nvos_sam3_soft_support_payload(
    payload: object,
    *,
    authority: PromptResponsibilityAuthority,
    ...
    if not isinstance(payload, dict) or set(payload) != ARTIFACT_KEYS:
        raise ValueError("NVOS SAM3 soft support artifact schema differs")
    if (
        ...
        raise ValueError("NVOS SAM3 soft support method or authority differs")
    ...
    if (
        ...
        raise ValueError("NVOS SAM3 soft pixel tensor digests differ")
    tensors = payload["tensors"]
    if not isinstance(tensors, dict) or set(tensors) != TENSOR_KEYS:
        raise ValueError("NVOS SAM3 soft support tensor schema differs")
    ...
    for name in (
        ...
        if (
            ...
            raise ValueError(f"NVOS SAM3 soft support tensor {name} is malformed")
    for name in ("primitive_probability", "query_compatibility"):
        if bool(((tensors[name] < 0) | (tensors[name] > 1)).any()):
            raise ValueError(f"NVOS SAM3 soft {name} is outside [0,1]")
    if bool(
        (
        ...
        raise ValueError("NVOS SAM3 soft signed evidence is outside [-1,1]")
    for name in ("reference_weight", "soft_positive_mass", "raw_negative_mass"):
        if bool((tensors[name] < 0).any()):
            raise ValueError(f"NVOS SAM3 soft {name} is negative")
    for name in (
        "capability_valid",
        "positive_exclusive",
        "negative_exclusive",
        ...
        if (
            ...
            raise ValueError(f"NVOS SAM3 soft support mask {name} is malformed")
    ...
    if bool(
        ...
        raise ValueError("NVOS SAM3 soft raw anchor partitions overlap")
    if not bool((tensors["primitive_probability"][positive] == 1).all()):
        raise ValueError("NVOS SAM3 soft positive anchors are not one")
    if not bool((tensors["primitive_probability"][negative] == 0).all()):
        raise ValueError("NVOS SAM3 soft negative anchors are not zero")
    if bool(
        (tensors["query_compatibility"][~tensors["capability_valid"]] != 0).any()
    ):
        raise ValueError("NVOS SAM3 soft invalid rows have compatibility")
    ...
    if (
        ...
        raise ValueError("NVOS SAM3 soft support tensor digests differ")
    if (
        expected_primitive_sha256 is not None
        and digests["primitive_probability"] != expected_primitive_sha256
    ):
        raise ValueError("NVOS SAM3 soft primitive probability differs")
...
def build(args: argparse.Namespace) -> dict[str, object]:
    ...
    if registration_sha256 != REGISTRATION_SHA256:
        raise ValueError("SAM3 soft-reliability registration differs")
    ...
    soft_mass = cache.adjoint(pixel_tensors["soft_positive_observation"]).weighted_sum
    ...
    signed[visible] = (soft_mass[visible] - negative_mass[visible]) / visible_mass[
        visible
    ]
    if bool(((signed < -1.0 - 1e-12) | (signed > 1.0 + 1e-12)).any()):
        raise ValueError("exact soft signed evidence exceeds probability bounds")
    signed = signed.clamp(-1.0, 1.0)
    reference_weight = soft_mass + negative_mass
    ...
    if device.type != "cuda" or not torch.cuda.is_available():
        raise ValueError("NVOS SAM3 soft support requires CUDA")
    ...
    tensors = {
        ...
        "soft_positive_mass": soft_mass.float().contiguous(),
    ...
    if sha256_file(output) != output_sha256:
        raise ValueError("NVOS SAM3 soft support changed across freeze and reload")
    report_payload = {
        "scene_id": args.scene_id,
        "artifact_type": ARTIFACT_TYPE,
        "method_contract": METHOD_CONTRACT,
        ...
        "mean_pixel_reliability": float(
            pixel_tensors["binary_entropy_reliability"].double().mean()
        ),
        "mean_soft_positive_observation": float(
            pixel_tensors["soft_positive_observation"].double().mean()
        ),
        "nonzero_soft_positive_fraction": float(
            (pixel_tensors["soft_positive_observation"] > 0).double().mean()
        ),
