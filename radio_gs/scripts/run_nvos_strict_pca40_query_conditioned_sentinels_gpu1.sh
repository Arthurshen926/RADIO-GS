...
# The PCA40 relation and K201 caches are CPU/query independent.  GPU work is
# limited to the frozen strict support solver and frozen target readout.
...
RUN_ROOT="${RUN_ROOT:-$REPO_ROOT/output/optimization_20260803/nvos_strict_cradio_dino_pca40_sentinels_v1}"
...
RELATION_REGISTRATION="$REPO_ROOT/paper/artifacts/cradio_dino_pca40_relation_cache_registration_20260803.json"
NVOS_REGISTRATION="$REPO_ROOT/paper/artifacts/nvos_strict_cradio_dino_pca40_experiment_registration_20260803.json"
...
mkdir -p "$RUN_ROOT"/{relation,knn,selectors,scores,reports,logs}
...
for scene in $SCENE_NAMES; do
  ...
  relation="$RUN_ROOT/relation/$scene.pt"
  ...
  for asset in "$capability" "$capability.json" "$graph" "$cache" "$cache_report" "$MANIFEST" "$RELATION_REGISTRATION" "$NVOS_REGISTRATION"; do
  ...
  if [[ ! -s "$relation" ]]; then
    echo "[$(date --iso-8601=seconds)] $scene: build query-independent C-RADIO primitive PCA40 relation on CPU"
    "$PYTHON" -m radio_gs.scripts.build_query_diffusion_pca40_relation_cache \
      --scene-id "$scene" \
      --capability-cache "$capability" \
      --experiment-registration "$RELATION_REGISTRATION" \
      --output "$relation" \
  ...
  if [[ ! -s "$knn" ]]; then
    ...
    "$PYTHON" -m radio_gs.scripts.build_query_diffusion_knn_cache \
      ...
      --num-neighbors 200 \
      --workers -1 \
      --experiment-registration "$NVOS_REGISTRATION" \
      >"$RUN_ROOT/logs/${scene}.knn.log" 2>&1
  fi

  if [[ ! -s "$selector" ]]; then
    echo "[$(date --iso-8601=seconds)] $scene: strict PCA40-relation query-conditioned support"
    run_guarded "${scene}.support" \
      radio_gs/scripts/build_nvos_strict_pca40_query_conditioned_support.py \
      --scene-id "$scene" \
      --experiment-registration "$NVOS_REGISTRATION" \
      --cache "$cache" \
  ...
  if [[ ! -s "$eval_report" ]]; then
    ...
    run_guarded "${scene}.score" \
      radio_gs/scripts/eval_nvos_strict_pca40_query_conditioned_support.py \
      --manifest "$MANIFEST" \
