#!/usr/bin/env python3
"""Materialize schema-union health V2 against the exact AcceptedV2 baseline."""
...
def descriptor_statistics(value: torch.Tensor) -> dict[str, float | int]:
    ...
    return {
        ...
        "centered_gram_effective_rank": trace * trace / frobenius_squared,
    }
...
def audit(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if output.exists() or output.is_symlink():
        raise FileExistsError(f"refuses to clobber health V2 audit: {output}")
    descriptor_record = {
    ...
    descriptor_raw, descriptor_sha, descriptor_path = load_torch_mapping(
        ...
        map_location="cpu",
        label="health V2 target descriptor",
    )
    descriptor = formal.validate_supported_target_descriptor(descriptor_raw)
    baseline_raw, baseline_sha, baseline_path = load_torch_mapping(
        baseline_record["path"],
        expected_sha256=baseline_record["sha256"],
        map_location="cpu",
        label="health V2 AcceptedV2 baseline",
    )
    ...
    if not all(alignment.values()):
        raise ValueError("health V2 descriptor/baseline alignment differs")
    ...
    payload = {
        ...
        "candidate_descriptor_schema": descriptor["schema"],
