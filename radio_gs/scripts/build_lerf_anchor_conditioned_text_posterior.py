...
def run(args):
    ...
    extent_metadata=extent.get("metadata",{})
    if not isinstance(extent_metadata,dict): raise ValueError("extent checkpoint metadata differs")
    ...
    with torch.inference_mode():
        ...
        for raw_query,adapted_query in zip(queries.to(device),adapted):
            ...
            authority=compile_bounded_spatial_authority(xyz_device,packet.peak_row,packet.local_radius,args.authority_radius_multiplier,args.maximum_authority_fraction);logits=decoder(latent,reliability,identity,packet,GaussianGeometry(xyz_device),authority)
    ...
    score_threshold=.5 if bool(extent_metadata.get("fixed_decision_thresholds",False)) else float(adapter_report["validation"]["threshold"])
    output=Path(args.output).resolve();write_torch_noclobber(output,{"schema":"radio_gs.lerf_anchor_conditioned_text_posterior.v1","schema_version":1,"scene":args.scene,"query_scores":torch.stack(posteriors,1),"identity_query_scores":torch.stack(identities,1),"anchor_rows":torch.stack(anchor_rows),"peak_rows":torch.tensor(peak_rows),"local_radii":torch.tensor(local_radii),"valid":torch.ones(latent.shape[0],dtype=torch.bool),"xyz":xyz,"metadata":{"query_names":names,"query_family":"text_object_extent","typed_posterior":"object_aware_universal_field_v2_text_object_posterior_anchor_conditioned_v1","score_threshold":score_threshold,"modality_specific_identity":True,"token_free_shared_extent":True,"peak_local_anchor":True,"authority_gated_replay":True,"maximum_authority_fraction":args.maximum_authority_fraction,"separate_identity_localization":True,"localization_authority":"field_siglip2_relevancy_identity","persistent_second_semantic_field":False,"benchmark_images_opened":False,"benchmark_masks_opened":False,"evaluation_rgb_opened":False,"query_text_opened_at_readout":True,"extent_model":extent_record,"text_adapter":adapter_record,"scene_manifest":seed_record,"text_embeddings":text_record,"scene_canonicalizer_index":args.scene_index,"unseen_identity_canonicalizer_explicitly_allowed":bool(args.allow_unseen_identity_canonicalizer)}})
