#!/usr/bin/env python3
"""Audit contrast V2.1 against its preregistered source-student envelope."""
...
from radio_gs.interfaces import factorized_native_contrast_v21_target_health_v3 as health_v3_formal
...
def _load_diagnostic(
    record: dict[str, str], *, label: str, validator, implementation_path: Path
) -> tuple[dict[str, Any], dict[str, str]]:
    raw, digest, source = load_json_object(
        record["path"], expected_sha256=record["sha256"], label=label
    )
    audit = validator(raw)
    producer = validate_file_record(audit["producer"], label=f"{label} producer")
    if producer != implementation_path:
        raise ValueError(f"{label} implementation differs")
    return audit, {"path": str(source), "sha256": digest}


def audit(args: argparse.Namespace) -> dict[str, Any]:
    output = Path(args.output).expanduser().resolve()
    if output.exists() or output.is_symlink():
        raise FileExistsError(f"refuses to clobber health V4 audit: {output}")
    prereg_record = {
    ...
    source_checkpoint_record = {
        "path": str(Path(args.source_contrast_v21_checkpoint).expanduser().resolve()),
        "sha256": str(args.expected_source_contrast_v21_checkpoint_sha256),
    }
    descriptor_record = {
    ...
    health_v3_record = {
        "path": str(Path(args.teacher_envelope_health_v3).expanduser().resolve()),
        "sha256": str(args.expected_teacher_envelope_health_v3_sha256),
    }
    ...
    if (
        source_result_record != formal.SOURCE_RESULT_RECORD
        or source_checkpoint_record != formal.SOURCE_CHECKPOINT_RECORD
    ):
        raise ValueError("health V4 source result/checkpoint singleton differs")
    source = descriptor_formal.validate_source_contrast_v21_result(
        source_result_record
    )
    if (
        source["source_only_passed"] is not True
        or source["checkpoint"]["verified_record"] != source_checkpoint_record
        or source["result"]["checkpoint"] != source_checkpoint_record
    ):
        raise ValueError("health V4 source promotion/checkpoint binding differs")

    health_v2, health_v2_record = _load_diagnostic(
        health_v2_record,
        label="Accepted-relative health V2 diagnostic",
        validator=health_v2_formal.validate_health_audit,
        implementation_path=health_v2_formal.HEALTH_AUDIT_IMPLEMENTATION_PATH,
    )
    health_v3, health_v3_record = _load_diagnostic(
        health_v3_record,
        label="teacher-envelope health V3 diagnostic",
        validator=health_v3_formal.validate_health_audit,
        implementation_path=health_v3_formal.HEALTH_AUDIT_IMPLEMENTATION_PATH,
    )
    ...
    descriptor_raw, descriptor_sha, descriptor_path = load_torch_mapping(
        ...
        map_location="cpu",
        label="contrast V2.1 health V4 target descriptor",
    )
    ...
    baseline_raw, baseline_sha, baseline_path = load_torch_mapping(
        ...
        map_location="cpu",
        label="contrast V2.1 health V4 AcceptedV2 baseline",
    )
    ...
    diagnostics_same_records = all(
        diagnostic["input_authority"]["target_descriptor"] == descriptor_record
        and diagnostic["input_authority"]["accepted_v2_baseline"]
        == baseline_record
        for diagnostic in (health_v2, health_v3)
    )
    diagnostics_same_statistics = all(
        diagnostic["candidate_statistics"] == candidate_stats
        and (
            diagnostic.get("baseline_statistics")
            or diagnostic.get("accepted_v2_diagnostic_statistics")
        )
        == baseline_stats
        for diagnostic in (health_v2, health_v3)
    )
    alignment = {
        ...
        "source_result_and_checkpoint_records_equal": (
            descriptor["input_authority"]["source_contrast_v21_result"]
            == source_result_record
            and descriptor["input_authority"]["source_contrast_v21_checkpoint"]
            == source_checkpoint_record
        ),
        "health_v2_and_v3_descriptor_baseline_records_equal": (
            diagnostics_same_records
        ),
        "health_v2_and_v3_candidate_statistics_equal": (
            diagnostics_same_statistics
        ),
    ...
    if not all(alignment.values()):
        raise ValueError("contrast V2.1 health V4 lineage/alignment differs")
    ...
    payload = {
        ...
        "input_authority": {
            ...
            "contrast_v21_source_result": source_result_record,
            "contrast_v21_source_checkpoint": source_checkpoint_record,
            ...
            "teacher_envelope_health_v3_diagnostic": health_v3_record,
        },
        ...
        "teacher_envelope_health_v3_status": health_v3["status"],
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--source-contrast-v21-checkpoint", required=True)
    parser.add_argument("--expected-source-contrast-v21-checkpoint-sha256", required=True)
    ...
    parser.add_argument("--teacher-envelope-health-v3", required=True)
    parser.add_argument("--expected-teacher-envelope-health-v3-sha256", required=True)
