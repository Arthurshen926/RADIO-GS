...
def run(args):
    ...
    latent_device=latent.to(device); reliability_device=reliability.to(device); xyz_device=xyz.to(device); baseline_device=baseline.to(device); scores=[]; raw_scores=[]; identities=[]
    with torch.inference_mode():
        ...
        for query in queries:
            ...
            probability=torch.sigmoid(logits).cpu(); primitive=prior.cpu(); raw_scores.append(probability)
            if "text_selective_blend" in model:
                calibration=model["text_selective_blend"]; weight=float(calibration["weight"])
                signed=weight*(probability-float(model["text_threshold"]))/float(calibration["posterior_scale"])+(1.0-weight)*(primitive-float(model["text_primitive_threshold"]))/float(calibration["primitive_scale"])
                scores.append(torch.sigmoid(signed))
            else: scores.append(probability)
            identities.append(torch.sigmoid(identity).cpu())
    output=Path(args.output).resolve(); write_torch_noclobber(output,{"schema":"radio_gs.lerf_query_native_text_object_posterior.v1","schema_version":1,"scene":args.scene,"query_scores":torch.stack(scores,1),"raw_decoder_query_scores":torch.stack(raw_scores,1),"identity_query_scores":torch.stack(identities,1),"valid":torch.ones(latent.shape[0],dtype=torch.bool),"xyz":xyz,"metadata":{"query_names":names,"query_family":"text_object_extent","typed_posterior":"object_aware_universal_field_v2_text_object_posterior_joint_query_native_v1","localization_authority":"field_siglip2_relevancy_identity","segmentation_authority":"joint_cross_view_object_extent_decoder_with_selective_primitive_fallback","score_threshold":0.5 if "text_selective_blend" in model else model.get("text_threshold"),"separate_identity_localization":True,"persistent_second_semantic_field":False,"benchmark_images_opened":False,"benchmark_masks_opened":False,"evaluation_rgb_opened":False,"query_text_opened_at_readout":True,"source_gate":report,"joint_model":model_record,"scene_manifest":seed_record,"text_embeddings":text_record,"scene_canonicalizer_index":args.scene_index}})
