#!/usr/bin/env python3
"""Source-only LOSO for transferable target-blind categorical calibration."""
...
from radio_gs.querying.typed_posteriors import MarginalCategoricalPosteriorV2
from radio_gs.scripts.train_source_categorical_posterior_v2 import (
    _write_json_noclobber,
    _write_torch_noclobber,
    load_manifest,
)
from radio_gs.scripts.train_source_post_spatial_text_posterior import FIT_SCENES


RECIPE = {
    "recipe_id": "source-marginal-categorical-posterior-v2-adamw-seed20260816-s300-v1",
    "seed": 20260816,
    "optimizer": "AdamW",
    "steps": 300,
    "learning_rate": 0.03,
    ...
    "target": "renormalized_soft_distribution_over_the_19_evaluation_classes",
    "ignored_rows": "zero_mass_in_the_19_evaluation_classes",
    "scene_statistics": "target_blind_valid_row_class_mean_and_population_std",
    "learned_parameters": "one_global_centering_strength_and_one_global_scaling_strength",
}
LOSO_SCHEMA = "radio_gs.source_marginal_categorical_posterior_v2_loso_result.v1"
CHECKPOINT_SCHEMA = "radio_gs.source_marginal_categorical_posterior_v2_checkpoint.v1"
FIT_RECEIPT_SCHEMA = "radio_gs.source_marginal_categorical_posterior_v2_fit_receipt.v1"


def foreground_targets(payload: Mapping[str, Any]) -> tuple[torch.Tensor, torch.Tensor]:
    target = torch.as_tensor(payload["semantic_class_distribution"]).float()
    mass = target.sum(dim=-1)
    normalized = target / mass.clamp_min(1.0e-12)[:, None]
    normalized = normalized.masked_fill(mass[:, None] <= 0, 0.0)
    return normalized.contiguous(), mass.contiguous()


def _row_weights(
    payload: Mapping[str, Any], target: torch.Tensor, foreground_mass: torch.Tensor
) -> torch.Tensor:
    ...
    base = (
        torch.as_tensor(payload["valid"]).float()
        * torch.as_tensor(payload["training_label_weight"]).float()
        * torch.as_tensor(payload["coverage"]).float()
        * authority
        * foreground_mass
    )
    ...
    inverse[present] = class_mass[present].reciprocal()
    balanced = base.double() * (target.double() * inverse[None, :]).sum(dim=-1)
    active = balanced > 0
    if not bool(active.any()):
        raise ValueError("source marginal categorical scene has no foreground rows")
    balanced[active] = balanced[active] / balanced[active].mean()
    return balanced.float().contiguous()


def weighted_foreground_metrics(
    ...
    if not values:
        raise ValueError("source marginal categorical scene has no evaluated classes")
    return {
...
def _predict(
    model: MarginalCategoricalPosteriorV2, payload: Mapping[str, Any]
) -> torch.Tensor:
    with torch.inference_mode():
        return model(
            payload["raw_positive_cosine"], valid=payload["valid"]
        ).prediction


def evaluate(
    model: MarginalCategoricalPosteriorV2,
    payloads: Sequence[Mapping[str, Any]],
) -> dict[str, Any]:
    rows = []
    for payload in payloads:
        target, foreground_mass = foreground_targets(payload)
        weight = _row_weights(payload, target, foreground_mass)
        baseline = torch.as_tensor(payload["raw_positive_cosine"]).argmax(dim=-1)
        ...
        rows.append(
            {
                ...
                "baseline": weighted_foreground_metrics(baseline, target, weight),
                "calibrated": weighted_foreground_metrics(calibrated, target, weight),
            }
        )
    baseline_iou = sum(row["baseline"]["miou"] for row in rows) / len(rows)
    calibrated_iou = sum(row["calibrated"]["miou"] for row in rows) / len(rows)
    baseline_acc = sum(row["baseline"]["macc"] for row in rows) / len(rows)
    calibrated_acc = sum(row["calibrated"]["macc"] for row in rows) / len(rows)
    return {
        "baseline_scene_macro_miou": baseline_iou,
        "calibrated_scene_macro_miou": calibrated_iou,
        "miou_delta": calibrated_iou - baseline_iou,
        "baseline_scene_macro_macc": baseline_acc,
        "calibrated_scene_macro_macc": calibrated_acc,
        "macc_delta": calibrated_acc - baseline_acc,
        "rows": rows,
    }
...
def _fit(
    payloads: Sequence[Mapping[str, Any]],
) -> tuple[MarginalCategoricalPosteriorV2, list[float]]:
    ...
    model = MarginalCategoricalPosteriorV2(num_classes=19).cpu()
    ...
    for payload in payloads:
        target, foreground_mass = foreground_targets(payload)
        prepared.append(
            (payload, target, _row_weights(payload, target, foreground_mass))
        )
    history = []
    for step in range(int(RECIPE["steps"])):
        ...
        for payload, target, weight in prepared:
            output = model(payload["raw_positive_cosine"], valid=payload["valid"])
            cross_entropy = -(target * F.log_softmax(output.logits, dim=-1)).sum(-1)
            losses.append(
                (cross_entropy * weight).sum() / weight.sum().clamp_min(1.0)
            )
        ...
        folds.append(
            {
                **fold,
                ...
                "learned_strengths": {
                    "centering": float(model.centering_parameter.tanh()),
                    "scaling": float(model.scaling_parameter.tanh()),
                },
...
def main() -> None:
    ...
    if args.mode == "loso":
        if not args.result or any((args.loso_authority, args.checkpoint, args.receipt)):
            raise ValueError("marginal categorical LOSO requires only --result")
        diagnostics = run_loso(payloads)
        result = {
            "schema": LOSO_SCHEMA,
            "schema_version": 1,
            "status": "complete_three_fold_source_only_marginal_categorical_loso",
            "recipe": dict(RECIPE),
            "dataset_manifest": {
    ...
    if not all((args.loso_authority, args.checkpoint, args.receipt)) or args.result:
        raise ValueError(
            "marginal categorical fit3 requires --loso-authority --checkpoint --receipt"
        )
    ...
    if (
        ...
        or loso.get("dataset_manifest", {}).get("sha256")
        != sha256_file(manifest_path)
        or loso.get("diagnostics", {}).get("all_passed") is not True
    ):
        raise PermissionError("marginal categorical fit3 is closed until source LOSO passes")
    model, history = _fit(payloads)
    fit_metrics = evaluate(model, payloads)
    if fit_metrics["miou_delta"] <= 0 or fit_metrics["macc_delta"] < -0.01:
        raise RuntimeError("marginal categorical fit3 source gate failed")
    ...
    receipt = {
        ...
        "schema_version": 1,
        "status": "complete_source_only_marginal_categorical_posterior_v2_fit3",
