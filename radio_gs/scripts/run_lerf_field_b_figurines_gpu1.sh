...
# Isolate the LERF effect of the post-gate Field-B checkpoint on figurines.
# Readout, support topology, text scorer, renderer, and evaluator are identical
# to the accepted Field-A comparison. No query-conditioned graph is run.
...
OUT=/mnt/pool/sqy/results/RADIO-GS/output/optimization_20260803/lerf_field_b_figurines_v1
FIELD=/mnt/pool/sqy/results/RADIO-GS/output/canonical_fields/figurines_compact_d256_l128_field_b_boundary_relation_seed0.pth
FIELD_SHA=f9b2d926c7a455acd6fcb2b87e3be9e385cd5cf2357204d0867546da93d9b710
...
DESCRIPTOR="$OUT/descriptors/figurines_field_b.pt"
SCORE_CACHE="$OUT/query_scores/figurines_field_b.pt"
...
for path in "$DESCRIPTOR" "$DESCRIPTOR.json" "$DESCRIPTOR.provenance.json" \
  "$OUT/descriptors/figurines_field_b_query.pt" "$SCORE_CACHE" \
  "$SCORE_CACHE.json" "$FIXED_RESULT" "$OTSU_RESULT" "$SCORE_RESULT"; do
  if [[ -e "$path" || -L "$path" ]]; then
    echo "refusing to overwrite Field-B LERF artifact: $path" >&2
    exit 2
  fi
done
...
guard descriptor \
  bash "$RUN_PYTHON" \
    ...
    --output "$DESCRIPTOR" \
    --query-output "$OUT/descriptors/figurines_field_b_query.pt" \
    --resume-dir "$OUT/resume_figurines" \
    --radio-batch-size 4096 \
    --semantic-batch-size 256 \
    --thermal-pacing-seconds-per-batch 0.25 \
    --device cuda:0 \
