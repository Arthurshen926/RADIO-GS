...
# Post-gate Field-B combination: rebuild the canonical capability graph from
# Field-B, then apply the exact already frozen LERF query-support operator.
...
OUT=/mnt/pool/sqy/results/RADIO-GS/output/optimization_20260803/lerf_field_b_query_conditioned_support_v1
...
SCORE=/mnt/pool/sqy/results/RADIO-GS/output/optimization_20260803/lerf_field_b_figurines_v1/query_scores/figurines_field_b.pt
...
CAPABILITY="$OUT/figurines_field_b_capability.pt"
GRAPH="$OUT/figurines_field_b_support_graph_k16.pt"
...
for path in "$CAPABILITY" "$CAPABILITY.json" "$GRAPH" "$GRAPH.json" \
  ...
  if [[ -e "$path" || -L "$path" ]]; then
    echo "refusing to overwrite Field-B support artifact: $path" >&2
    exit 2
  fi
done
