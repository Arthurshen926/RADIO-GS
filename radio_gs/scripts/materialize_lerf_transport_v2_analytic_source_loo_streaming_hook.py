#!/usr/bin/env python3
"""Scalar-only streaming hook for analytic transport-v2 source LOO.

This is a versioned successor to the sealed descriptor-materializing hook.  It
accepts the same already-resident source tensors, but delegates to the
analytic cosine/order-statistic engine and never makes candidate descriptors
durable.
"""
...
from radio_gs.interfaces.lerf_scale_residual_shrinkage_transport_order_statistic_v2 import (
...
HOOK_SCHEMA = "radio_gs.lerf_transport_v2_analytic_source_loo_streaming_hook.v2"
...
def hook_contract() -> dict[str, Any]:
    return {
        ...
        "streaming": {
            "row_chunked_scalar_compute": True,
            "candidate_descriptors_materialized": False,
...
def capture_source_only_transport_v2_loo(
    *,
    ...
    if not isinstance(scene_id, str) or not scene_id or scene_id.strip() != scene_id:
        raise ValueError("analytic transport-v2 source LOO scene id differs")
...
def validate_streaming_hook_capture(value: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise ValueError("analytic transport-v2 source LOO streaming capture differs")
