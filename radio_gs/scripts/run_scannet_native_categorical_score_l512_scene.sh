...
VARIANT=${VARIANT:-native_categorical_score_l512_v3_gate_replay}
GATE_MODE=${GATE_MODE:-replay}
RUN=$ROOT/optimization_20260824/native_multiteacher_v1/$SCENE/$VARIANT
...
for value in "$FIELD" "$BASE" "$MEMBERSHIP" "$TEACHER" "$CHECKPOINT" "$CONFIG" "$LABEL" "$P19" "$P15" "$P10" "$R19" "$R15" "$R10"; do
...
case "$GATE_MODE" in
  replay) GATE_ARGS=(--replay-teacher-eligibility) ;;
  predicted)
    [[ -s "$UNIVERSAL" ]] || { echo "incomplete input: $UNIVERSAL" >&2; exit 1; }
    GATE_ARGS=(--predict-teacher-eligibility --universal-field "$UNIVERSAL" --expected-universal-field-sha256 "$(sha "$UNIVERSAL")")
    ;;
  *) echo "unsupported GATE_MODE: $GATE_MODE" >&2; exit 2 ;;
esac
if [[ ! -s "$CACHE" ]]; then
  CUDA_VISIBLE_DEVICES=$GPU OMP_NUM_THREADS=8 MKL_NUM_THREADS=8 \
    bash "$REPO/radio_gs/scripts/run_repo_python.sh" \
      -m radio_gs.scripts.train_scannet_frozen_l512_native_categorical_score_decoder \
      ...
      --device cuda:0 --steps 600 --hidden-dim 256 "${GATE_ARGS[@]}"
fi
if [[ ! -s "$RESULT" ]]; then
