...
SAM_K=${SAM_K:-12}
SAM_RADIUS=${SAM_RADIUS:-0.10}
SAM_SIMILARITY=${SAM_SIMILARITY:-0.65}
SEED_MARGIN=${SEED_MARGIN:-0.04}
UPDATE_MARGIN=${UPDATE_MARGIN:-0.04}
SEMANTIC_TOLERANCE=${SEMANTIC_TOLERANCE:-0.025}
CONSENSUS=${CONSENSUS:-0.70}
ITERATIONS=${ITERATIONS:-6}
SAVE_DEVELOPMENT_SCORE_CACHE=${SAVE_DEVELOPMENT_SCORE_CACHE:-0}

EXTRA_ARGS=()
if [[ "$SAVE_DEVELOPMENT_SCORE_CACHE" == "1" ]]; then
  EXTRA_ARGS+=(--save_development_score_cache)
fi
...
CUDA_VISIBLE_DEVICES="$PHYSICAL_GPU" \
  ...
  --sam_region_k "$SAM_K" \
  --sam_region_radius "$SAM_RADIUS" \
  --sam_region_similarity_threshold "$SAM_SIMILARITY" \
  --sam_region_alpha 0 \
  --sam_instance_topology \
  --sam_instance_seed_margin "$SEED_MARGIN" \
  --sam_instance_update_margin "$UPDATE_MARGIN" \
  --sam_instance_semantic_tolerance "$SEMANTIC_TOLERANCE" \
  --sam_instance_consensus "$CONSENSUS" \
  --sam_instance_iterations "$ITERATIONS" \
  "${EXTRA_ARGS[@]}" \
  --device cuda
