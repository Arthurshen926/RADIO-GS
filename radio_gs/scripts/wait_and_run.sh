...
if [[ $# -lt 2 ]]; then
  echo "Usage: bash radio_gs/scripts/wait_and_run.sh <log_path> <command...>" >&2
  exit 1
fi
...
echo "[$(timestamp)] Waiting for GPU before running: ${CMD[*]}" | tee -a "$LOG_PATH"
echo "[$(timestamp)] min_free=${MIN_FREE_MIB}MiB max_util=${MAX_UTIL}% gpus=$GPU_LIST" | tee -a "$LOG_PATH"
  ...
  while IFS=',' read -r gpu_idx free_mib util_pct; do
    ...
    if [[ "$free_mib" =~ ^[0-9]+$ ]] && [[ "$util_pct" =~ ^[0-9]+$ ]]; then
      if (( free_mib >= MIN_FREE_MIB && util_pct <= MAX_UTIL )); then
        ...
        echo "[$(timestamp)] Launching on GPU $gpu_idx (free=${free_mib}MiB util=${util_pct}%)" | tee -a "$LOG_PATH"
        (
