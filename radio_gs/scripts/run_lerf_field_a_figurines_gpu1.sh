...
# Isolate the LERF effect of the frozen Field-A checkpoint on figurines.
# The SurfaceRegion readout, support topology, text scorer, renderer, and
# evaluator are identical to the initial-field baseline. No query graph is run.
...
OUT=/mnt/pool/sqy/results/RADIO-GS/output/optimization_20260803/lerf_field_a_figurines_v1
...
DESCRIPTOR="$OUT/descriptors/figurines_field_a.pt"
SCORE_CACHE="$OUT/query_scores/figurines_field_a.pt"
...
for path in "$DESCRIPTOR" "$DESCRIPTOR.json" "$DESCRIPTOR.provenance.json" \
  "$OUT/descriptors/figurines_field_a_query.pt" "$SCORE_CACHE" \
  "$SCORE_CACHE.json" "$FIXED_RESULT" "$OTSU_RESULT" "$SCORE_RESULT"; do
  if [[ -e "$path" || -L "$path" ]]; then
    echo "refusing to overwrite Field-A LERF artifact: $path" >&2
    exit 2
  fi
done
...
guard descriptor \
  bash "$RUN_PYTHON" \
    ...
    --output "$DESCRIPTOR" \
    --query-output "$OUT/descriptors/figurines_field_a_query.pt" \
