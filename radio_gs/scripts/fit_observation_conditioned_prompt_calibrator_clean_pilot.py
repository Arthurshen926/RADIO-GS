#!/usr/bin/env python3
"""Fit and evaluate the observation-conditioned four-parameter calibrator."""
...
def _validate_authority(args: argparse.Namespace) -> tuple[dict, dict, dict]:
    authority, authority_sha, authority_path = load_json_object(
        ...
        label="observation-conditioned calibrator execution authority",
    )
    if (
        authority.get("schema")
        != "radio_gs.observation_conditioned_prompt_calibrator.execution_authority.v4"
        or authority.get("schema_version") != 4
        or authority.get("scene_id") != v1.SCENE_ID
    ):
        raise ValueError("observation-conditioned calibrator execution contract differs")
    inputs = authority.get("inputs")
    if not isinstance(inputs, dict):
        raise ValueError("observation-conditioned calibrator inputs differ")
    bindings = {
        "preregistration": args.preregistration,
        "v3_diagnostic_result": args.v3_result,
        ...
        "v3_architecture_module": Path(__file__).resolve().parents[1]
        / "querying"
        / "prompt_mode_logit_calibrator.py",
        "v4_calibrator_module": Path(__file__).resolve().parents[1]
        / "querying"
        / "observation_conditioned_prompt_calibrator.py",
        "implementation": Path(__file__).resolve(),
    }
    expected_cli = {
        "preregistration": args.expected_preregistration_sha256,
        "v3_diagnostic_result": args.expected_v3_result_sha256,
    ...
    if (
        ...
        raise ValueError("observation-conditioned calibrator source-access contract differs")
    for output in (args.calibrator_output, args.fit_receipt, args.result_output):
        path = Path(output)
        if path.exists() or path.is_symlink():
            raise FileExistsError(f"refusing to overwrite V4 output: {path}")
    prereg, _, _ = load_json_object(
        args.preregistration,
        expected_sha256=args.expected_preregistration_sha256,
        label="observation-conditioned calibrator preregistration",
    )
    if (
        prereg.get("schema")
        != "radio_gs.observation_conditioned_prompt_calibrator.clean_scannet_preregistration.v4"
        or prereg.get("schema_version") != 4
        or prereg.get("scene_id") != v1.SCENE_ID
        or prereg.get("method", {}).get("parameter_count") != 4
    ):
        raise ValueError("observation-conditioned calibrator preregistration differs")
...
def main() -> None:
    ...
    parser.add_argument("--expected-v3-result-sha256", required=True)
    ...
    calibrator = ObservationConditionedPromptCalibratorV4().to(
    ...
    for mode in PROMPT_MODES:
        prompt_data = [
            (
                value["score"].to(device=device, dtype=torch.float64),
                value["label"].to(device=device, dtype=torch.float64),
            )
            for key, value in sorted(train.items())
            if key[1] == mode
        ]
        if len(prompt_data) != len(train_ids):
            raise RuntimeError(f"{mode} train prompt count differs")
        scores = torch.cat([value[0] for value in prompt_data])
        labels = torch.cat([value[1] for value in prompt_data])
        ...
        def registered_loss() -> torch.Tensor:
            if mode == "full_mask":
                return F.binary_cross_entropy_with_logits(
                    branch.calibrated_logit(scores), labels
                )
            return prompt_case_control_logistic_loss(
                [
                    (branch.calibrated_logit(prompt_scores), prompt_labels)
                    for prompt_scores, prompt_labels in prompt_data
                ]
            )
        ...
        initial_registered_loss = float(closure().detach())
        optimizer.step(closure)
        with torch.no_grad():
            final_logit = branch.calibrated_logit(scores)
            final_registered_loss = float(registered_loss())
            final_pixel_micro_bce = float(
                F.binary_cross_entropy_with_logits(final_logit, labels)
            )
        ...
        fit_by_mode[mode] = {
            ...
            "initial_registered_loss": initial_registered_loss,
            "loss_contract": "ordinary_pixel_micro_bce"
            if mode == "full_mask"
            else "equal_class_within_prompt_then_equal_prompt_case_control_logistic",
            "final_registered_loss": final_registered_loss,
            "final_pixel_micro_bce": final_pixel_micro_bce,
    ...
    calibrator_payload = {
        "schema": "radio_gs.observation_conditioned_prompt_calibrator.checkpoint.v4",
        "schema_version": 4,
        "state_dict": {
    ...
    result = {
        "schema": "radio_gs.observation_conditioned_prompt_calibrator.clean_scannet_result.v4",
        "schema_version": 4,
        "scene_id": v1.SCENE_ID,
        "method": "ObservationConditionedPromptCalibratorV4",
        "parameter_count": 4,
        "graph": "off",
        ...
        "authority": {
            ...
            "v3_diagnostic_result_sha256": args.expected_v3_result_sha256,
            "v1_result_sha256": args.expected_v1_result_sha256,
            "v1_checkpoint_sha256": args.expected_v1_checkpoint_sha256,
            "calibrator_checkpoint": {
                "path": str(args.calibrator_output.resolve()),
        ...
        "iteration_status": {
            "same_scene_validation_was_seen_for_v2_and_v3_method_diagnosis": True,
