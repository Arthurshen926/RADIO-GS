"""Schema-union query-free anti-collapse audit for native target descriptors.

Health V1 remains immutable and accepts only its original descriptor schema.
This V2 adds an explicit, fail-closed dispatch for the frozen V1 descriptor and
the independent contrast-V2.1 descriptor.  No structural duck typing is used.
"""
...
from radio_gs.interfaces import factorized_native_contrast_v21_target_descriptor as contrast_formal
from radio_gs.interfaces import factorized_native_target_descriptor as v1_formal
from radio_gs.utils.immutable_artifacts import canonical_json_sha256


HEALTH_AUDIT_SCHEMA = "radio_gs.factorized_native_target_health_audit.v2"
HEALTH_AUDIT_SCHEMA_VERSION = 2
HEALTH_AUDIT_IMPLEMENTATION_PATH = (
    Path(__file__).resolve().parents[1]
    / "scripts/audit_factorized_native_target_descriptor_health_v2.py"
)
SUPPORTED_DESCRIPTOR_SCHEMAS = (
    v1_formal.TARGET_DESCRIPTOR_SCHEMA,
    contrast_formal.TARGET_DESCRIPTOR_SCHEMA,
)
...
def validate_supported_target_descriptor(value: object) -> dict[str, Any]:
    if not isinstance(value, Mapping):
        raise ValueError("health V2 target descriptor must be a mapping")
    schema = value.get("schema")
    if schema == v1_formal.TARGET_DESCRIPTOR_SCHEMA:
        return v1_formal.validate_target_descriptor_authority(value)
    if schema == contrast_formal.TARGET_DESCRIPTOR_SCHEMA:
        return contrast_formal.validate_target_descriptor_authority(value)
    raise ValueError("health V2 target descriptor schema is not supported")
...
def health_contract() -> dict[str, Any]:
    return {
        ...
        "candidate": {
            "dispatch": "exact_schema_union_no_structural_field_probing",
            "supported_descriptor_schemas": list(SUPPORTED_DESCRIPTOR_SCHEMAS),
        },
...
def _validate_statistics(value: object, *, label: str) -> dict[str, float | int]:
    ...
    if not isinstance(value, Mapping) or set(value) != required:
        raise ValueError(f"{label} health V2 statistics fields differ")
    result = dict(value)
    if (
        ...
        raise ValueError(f"{label} health V2 statistics differ")
    return result
...
def expected_checks(
    ...
    return {
        ...
        "sampled_pair_mean_not_more_collapsed": (
            ...
            <= float(baseline["pair_cosine_mean"]) + PAIR_MEAN_ABSOLUTE_SLACK
        ),
        "sampled_pair_p90_not_more_collapsed": (
            float(candidate["pair_cosine_p90"])
            <= float(baseline["pair_cosine_p90"]) + PAIR_P90_ABSOLUTE_SLACK
        ),
...
def validate_health_audit(value: object, *, require_pass: bool = False) -> dict[str, Any]:
    ...
    if not isinstance(value, Mapping) or set(value) != required:
        raise ValueError("factorized-native health V2 audit fields differ")
    audit = dict(value)
    if (
        ...
        or audit["candidate_descriptor_schema"] not in SUPPORTED_DESCRIPTOR_SCHEMAS
        ...
        raise ValueError("factorized-native health V2 audit header differs")
    audit["producer"] = record(audit["producer"], label="health V2 audit producer")
    inputs = audit["input_authority"]
    if not isinstance(inputs, Mapping) or set(inputs) != {
        "target_descriptor",
        "accepted_v2_baseline",
    }:
        raise ValueError("factorized-native health V2 audit inputs differ")
    audit["input_authority"] = {
        name: record(inputs[name], label=f"health V2 audit {name}")
    ...
    for name in ("descriptor_channel_sha256", "baseline_channel_sha256"):
        ...
        if (
            ...
            raise ValueError(f"factorized-native health V2 {name} differs")
    expected_alignment = {
    ...
    if audit["alignment_audit"] != expected_alignment:
        raise ValueError("factorized-native health V2 alignment audit differs")
    candidate = _validate_statistics(audit["candidate_statistics"], label="candidate")
    ...
    if (
        ...
        raise ValueError("factorized-native health V2 sampling axes differ")
    ...
    if (
        ...
        raise ValueError("factorized-native health V2 decision differs")
...
__all__ = [
    ...
    "validate_supported_target_descriptor",
]
