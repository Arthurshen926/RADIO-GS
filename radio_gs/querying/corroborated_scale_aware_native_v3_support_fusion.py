"""Native-V3 support fusion gated by independent target-side unary evidence.

Relations may consolidate an already semantically witnessed target region, but
may not manufacture the first foreground decision inside that region.  A
non-seed region is therefore eligible for the unchanged noisy-OR completion
only when at least one valid primitive in its semantic core already reaches
the frozen 0.60 boundary at the region's own registered scale.
"""
...
READOUT_SCHEMA = "radio_gs.corroborated_scale_aware_native_v3_support_fusion.v1"
...
def readout_contract() -> dict[str, Any]:
    return {
        ...
        "seed": (
            "highest_region_anchor_unary_restricted_to_frozen_query_selected_scale"
        ),
        "relation": {
        ...
        "target_corroboration": (
            "at_least_one_valid_semantic_core_primitive_at_registered_scale_"
            "already_reaches_semantic_boundary"
        ),
        "completion": legacy.readout_contract()["completion"],
        "invariants": {
            ...
            "relation_cannot_create_first_target_region_foreground": True,
...
class CorroboratedScaleAwareNativeV3Fusion:
    ...
    corroborated_relation_region_masks: torch.Tensor
    relation_path_support: torch.Tensor
    changed_primitive_query_cells: int


def corroborated_scale_aware_native_v3_support_fusion(
    *,
    primitive_relevance_by_scale: torch.Tensor,
...
) -> CorroboratedScaleAwareNativeV3Fusion:
    (
        relevance,
        selected_scale,
        valid,
    ...
    for query in range(query_count):
        ...
        for region in selected_regions:
            ...
            # No new parameter: reuse the frozen semantic boundary.  This
            # evidence precedes and is independent of relation completion.
            if not bool((local >= SEMANTIC_BOUNDARY).any()):
                continue
    ...
    if (
        ...
        raise RuntimeError("corroborated scale-aware native-V3 invariant failed")
    return CorroboratedScaleAwareNativeV3Fusion(
        ...
        corroborated_relation_region_masks=corroborated.contiguous(),
