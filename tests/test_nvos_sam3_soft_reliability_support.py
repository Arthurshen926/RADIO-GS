...
def _payload(authority: PromptResponsibilityAuthority) -> dict:
    ...
    tensors = {
        ...
        "soft_positive_mass": torch.tensor([2.0, 0.0, 1.0, 0.0]),
...
def test_soft_reliability_contract_has_no_threshold_or_scene_rule():
    assert METHOD_CONTRACT["Li_threshold"] == "forbidden"
    assert "parameter_free" in METHOD_CONTRACT["reliability"]
    assert METHOD_CONTRACT["target_dependent_tuning"] is False
    assert METHOD_CONTRACT["connected_selection"] == "none"


def test_soft_reliability_validator_recomputes_pixel_formula():
...
@pytest.mark.parametrize(
    "tamper",
    ["formula", "pixel_bundle", "primitive", "registration", "target", "schema"],
)
def test_soft_reliability_validator_fails_closed(tamper):
    authority = _authority()
    payload = _payload(authority)
    if tamper == "formula":
        payload["pixel_tensors"]["binary_entropy_reliability"][0, 1] += 0.01
    elif tamper == "pixel_bundle":
        payload["pixel_tensor_bundle_sha256"] = "0" * 64
...
def test_repository_registration_hash_matches_soft_followup_binding():
    import hashlib
    ...
    path = (
        ...
        / "paper"
        / "artifacts"
        / "nvos_sam3_soft_reliability_followup_registration_20260803.json"
    )
