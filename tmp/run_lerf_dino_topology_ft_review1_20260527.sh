...
GPU="${1:-3}"
VARIANT="${2:-dino_topology_relcv_textheat_ft40_20260527}"
SCENES=("${@:3}")
if [[ ${#SCENES[@]} -eq 0 ]]; then
...
# LERF feature filenames may preserve sparse original frame IDs while
# traj_w_c.txt is dense in feature order. Keeping the full ordered list lets
# the pose loader use its dense-order fallback and preserves adjacent batches
# for cross-view relation distillation.
Path(train_out).write_text("\n".join(str(i) for i in frame_ids) + "\n", encoding="utf-8")
Path(val_out).write_text("\n".join(str(i) for i in frame_ids) + "\n", encoding="utf-8")
print(scene, len(frame_ids), len(frame_ids), len(frame_ids))
PY
...
PY
  ...
  echo "[lerf-dino-topology] gpu=${GPU} scene=${scene} cfg=${cfg}"
  ...
  echo "[lerf-dino-topology-eval] gpu=${GPU} scene=${scene}"
  CUDA_VISIBLE_DEVICES="${GPU}" bash radio_gs/scripts/run_repo_python.sh \
