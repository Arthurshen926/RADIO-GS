...
if [[ $# -lt 2 ]]; then
  echo "Usage: bash tmp/run_sam3_box_global_selector_sweep_20260516.sh <gpu> <sam3_box_padding> [scene ...]" >&2
  exit 1
fi
...
OUT_ROOT="output/radio_gs/lerf_direct3d_sam3_box_pad${PAD}_global_selector_${STAMP}"
LOG_DIR="output/radio_gs/logs/sam3_box_global_selector_${STAMP}"
...
run_scene() {
  local scene="$1"
  local log_path="$LOG_DIR/pad${PAD}_${scene}_gpu${GPU}.log"
  echo "[$(date '+%F %T')] start pad=${PAD} scene=${scene} gpu=${GPU}" | tee -a "$LOG_DIR/queue.log"
  CUDA_VISIBLE_DEVICES="$GPU" PYTHONUNBUFFERED=1 bash radio_gs/scripts/run_repo_python.sh \
    ...
    --sam3_box_padding "$PAD" \
    --sam3_checkpoint_path checkpoints/sam3_modelscope/sam3.pt \
    --sam3_resolution 1008 \
    --sam3_min_initial_iou 0.05 \
    --chunk_size 8192 \
    --gpu 0 \
    ${EXTRA_EVAL_ARGS:-} 2>&1 | tee "$log_path"
}
...
echo "[$(date '+%F %T')] complete pad=${PAD} gpu=${GPU} out=${OUT_ROOT}" | tee -a "$LOG_DIR/queue.log"
