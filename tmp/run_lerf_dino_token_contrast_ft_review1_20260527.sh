...
GPU="${1:-3}"
VARIANT="${2:-dino_tokencontrast_relcv_textheat_ft30_20260527}"
SCENES=("${@:3}")
if [[ ${#SCENES[@]} -eq 0 ]]; then
...
cfg.update(
    {
        ...
        "radio_adaptor_token_contrast_weight": 0.002,
...
PY
  ...
  echo "[lerf-dino-token-contrast] gpu=${GPU} scene=${scene} cfg=${cfg}"
  ...
  echo "[lerf-dino-token-contrast-eval] gpu=${GPU} scene=${scene}"
  CUDA_VISIBLE_DEVICES="${GPU}" bash radio_gs/scripts/run_repo_python.sh \
