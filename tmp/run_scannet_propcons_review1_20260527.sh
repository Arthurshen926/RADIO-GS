...
GPU="${1:-4}"
VARIANT="${2:-v74_propcons005_from_v67dinocv_ft10}"
BATCH_SIZE="${3:-2}"
DIRECT_POINT_SAMPLE_COUNT="${4:-32768}"
CFG_ROOT="tmp/scannet_propcons_${VARIANT}"
SCENES=(
  scene0000_00
  scene0062_00
  scene0070_00
...
for scene in "${SCENES[@]}"; do
  ...
  echo "[scannet-propcons] gpu=${GPU} scene=${scene} cfg=${cfg}"
...
eval_out="output/scannet_pointcloud_eval/vala8_${VARIANT}_knn16_cand80_scene_mean_a045_smoothk12a1_propconf_vxl005_a02_c060"
CUDA_VISIBLE_DEVICES="${GPU}" bash radio_gs/scripts/run_repo_python.sh \
  ...
  --proposal_smoothing_gate low_confidence \
...
bash radio_gs/scripts/run_repo_python.sh \
  ...
  --label "RADIO-GS VALA8 proposal-consistency + DINO-CV contextual kNN16/cand80 + spatial/proposal readout" \
  ...
  --expect_arg proposal_smoothing_gate=low_confidence \
