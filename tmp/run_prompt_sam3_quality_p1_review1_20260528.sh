...
GPU="${1:-1}"
VARIANT="${2:-trainviews_directcoarse_quality_w020_aug_e70_20260528}"
shift 2 || true
SCENES=("$@")
if [[ ${#SCENES[@]} -eq 0 ]]; then
...
for SCENE in "${SCENES[@]}"; do
  ...
  EVAL_DIR="output/radio_gs/lerf_${SCENE}_direct_prompt_sam3_${VARIANT}_q055_eval"

  echo "[p1-prompt-sam3-quality] train gpu=${GPU} scene=${SCENE} variant=${VARIANT}"
  ...
  echo "[p1-prompt-sam3-quality] eval gpu=${GPU} scene=${SCENE}"
  CUDA_VISIBLE_DEVICES="${GPU}" bash radio_gs/scripts/run_repo_python.sh \
