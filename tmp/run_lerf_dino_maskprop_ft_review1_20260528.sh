...
GPU="${1:-3}"
VARIANT="${2:-dino_maskprop_relcv_textheat_ft35_20260528}"
BATCH_SIZE="${3:-4}"
SCENES=("${@:4}")
if [[ ${#SCENES[@]} -eq 0 ]]; then
...
EPOCHS="${EPOCHS:-35}"
ALIGN_WEIGHT="${ALIGN_WEIGHT:-0.005}"
RELATION_WEIGHT="${RELATION_WEIGHT:-0.006}"
TOKEN_CONTRAST_WEIGHT="${TOKEN_CONTRAST_WEIGHT:-0.001}"
CROSS_VIEW_WEIGHT="${CROSS_VIEW_WEIGHT:-0.0005}"
CROSS_VIEW_PROP_WEIGHT="${CROSS_VIEW_PROP_WEIGHT:-0.002}"
CROSS_VIEW_MASK_PROP_WEIGHT="${CROSS_VIEW_MASK_PROP_WEIGHT:-0.003}"
...
PY
  ...
  bash radio_gs/scripts/run_repo_python.sh - \
    ...
    "${EPOCHS}" "${ALIGN_WEIGHT}" "${RELATION_WEIGHT}" "${TOKEN_CONTRAST_WEIGHT}" \
    "${CROSS_VIEW_WEIGHT}" "${CROSS_VIEW_PROP_WEIGHT}" "${CROSS_VIEW_MASK_PROP_WEIGHT}" \
    "${CROSS_VIEW_ANCHOR_STRATEGY}" <<'PY'
import sys
import yaml
...
PY
  ...
  echo "[lerf-dino-maskprop] gpu=${GPU} scene=${scene} batch=${BATCH_SIZE} cfg=${cfg}"
  ...
  echo "[lerf-dino-maskprop-eval] gpu=${GPU} scene=${scene}"
  CUDA_VISIBLE_DEVICES="${GPU}" bash radio_gs/scripts/run_repo_python.sh \
