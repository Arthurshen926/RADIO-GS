...
GPU="${1:-1}"
VARIANT="${2:-trainviews_directcoarse_quality_aligned_support_20260528}"
shift 2 || true
SCENES=("$@")
if [[ ${#SCENES[@]} -eq 0 ]]; then
...
cfg_for_scene() {
  case "$1" in
    ramen)
      echo "tmp/lerf_adjoint_vpr_supportdistill_zscore_w030_propcontrast_p0_ft18_mf16_20260528/lerf_ramen_adjoint_vpr_supportdistill_zscore_w030_propcontrast_p0_ft18_mf16_20260528.yaml"
      ;;
    figurines)
      echo "tmp/lerf_adjoint_vpr_supportdistill_zscore_w020_p0_ft12_mf16_20260528/lerf_figurines_adjoint_vpr_supportdistill_zscore_w020_p0_ft12_mf16_20260528.yaml"
      ;;
    *)
...
ckpt_for_scene() {
  case "$1" in
    ramen)
      echo "output/radio_gs/lerf_ramen_adjoint_vpr_supportdistill_zscore_w030_propcontrast_p0_ft18_mf16_20260528/checkpoints/best.pth"
      ;;
    figurines)
      echo "output/radio_gs/lerf_figurines_adjoint_vpr_supportdistill_zscore_w020_p0_ft12_mf16_20260528/checkpoints/best.pth"
      ;;
    *)
      echo "unknown scene: $1" >&2
      return 2
...
for SCENE in "${SCENES[@]}"; do
  ...
  EVAL_DIR="output/radio_gs/lerf_${SCENE}_direct_prompt_sam3_${VARIANT}_topratio_eval"

  echo "[p1-prompt-sam3-aligned] train gpu=${GPU} scene=${SCENE} variant=${VARIANT}"
  CUDA_VISIBLE_DEVICES="${GPU}" bash radio_gs/scripts/run_repo_python.sh \
    ...
    --support_outside_weight 0.20 \
    --support_outside_dilate 8 \
    --quality_weight 0.20 \
    --quality_mask_threshold -1.0 \
    --coarse_dilate 3 \
    --coarse_threshold 0.5 \
    --coarse_aug_dropout_prob 0.10 \
    --coarse_aug_false_positive_prob 0.0007 \
  ...
  echo "[p1-prompt-sam3-aligned] eval gpu=${GPU} scene=${SCENE}"
  CUDA_VISIBLE_DEVICES="${GPU}" bash radio_gs/scripts/run_repo_python.sh \
    ...
    --sam3_prompt_mask_head_min_quality 0.55 \
