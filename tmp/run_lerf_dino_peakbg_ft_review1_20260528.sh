...
GPU="${1:-3}"
VARIANT="${2:-dino_peakbg_maskprop_ft25_b4_20260528}"
BATCH_SIZE="${3:-4}"
SCENES=("${@:4}")
if [[ ${#SCENES[@]} -eq 0 ]]; then
...
PEAK_BACKGROUND_WEIGHT="${PEAK_BACKGROUND_WEIGHT:-0.01}"
...
PY
  ...
  bash radio_gs/scripts/run_repo_python.sh - \
    ...
    "${PEAK_BACKGROUND_WEIGHT}" "${CROSS_VIEW_WEIGHT}" "${CROSS_VIEW_PROP_WEIGHT}" \
...
PY
  ...
  echo "[lerf-dino-peakbg] gpu=${GPU} scene=${scene} batch=${BATCH_SIZE} cfg=${cfg}"
  ...
  echo "[lerf-dino-peakbg-eval] gpu=${GPU} scene=${scene}"
  CUDA_VISIBLE_DEVICES="${GPU}" bash radio_gs/scripts/run_repo_python.sh \
